...
const lines = [
	...
	'\t},',
	'\t"byNode": {',
	...Object.keys( byNode ).sort().map( ( node ) => `\t\t${JSON.stringify( node )}: ${JSON.stringify( byNode[ node ].sort() )},` ),
	'\t},',
	'};',
	'',
...
console.log( `Wrote ${Object.keys( resolved ).length} nodedefs (${Object.keys( byNode ).length} node categories) to ${path.relative( process.cwd(), outputPath )}` );
