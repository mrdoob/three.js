		...
		static mergeObject(object) {
			...
			function extractGroup(geometry, group, elementSize, isConditionalLine) {
				...
				const originalNormals =
					elementSize === 3 ? geometry.getAttribute("normal").array : null;
				const numVertsGroup = Math.min(
					group.count,
					Math.floor(originalPositions.length / 3) - group.start
				);
				...
				const normals =
					originalNormals !== null
						? originalNormals.subarray(vertStart, vertEnd)
						: null;
				...
				if (isConditionalLine) {
					const controlArray0 = geometry
						.getAttribute("control0")
						.array.subarray(vertStart, vertEnd);
					const controlArray1 = geometry
						.getAttribute("control1")
						.array.subarray(vertStart, vertEnd);
					const directionArray = geometry
						.getAttribute("direction")
						.array.subarray(vertStart, vertEnd);
					newGeometry.setAttribute(
						"control0",
						new THREE.BufferAttribute(controlArray0, 3, false)
					);
					newGeometry.setAttribute(
						"control1",
						new THREE.BufferAttribute(controlArray1, 3, false)
					);
					newGeometry.setAttribute(
						"direction",
						new THREE.BufferAttribute(directionArray, 3, false)
					);
				}
				...
				if (c.isMesh | c.isLineSegments) {
					...
					if (Array.isArray(c.material)) {
						for (const groupIndex in geometry.groups) {
							...
							const newGeometry = extractGroup(
								geometry,
								group,
								elemSize,
								c.isConditionalLine
			...
			for (const linesMaterialsId of linesMaterialsIds) {
				...
				mergedObject.add(
					new THREE.LineSegments(mergedGeometry, lineGeometry.mat)
				);
			}
			...
			for (const condLinesMaterialsId of condLinesMaterialsIds) {
				...
				const mergedGeometry = THREE.mergeBufferGeometries(
					condLineGeometry.arr
				);
				const condLines = new THREE.LineSegments(
					mergedGeometry,
					condLineGeometry.mat
				);
