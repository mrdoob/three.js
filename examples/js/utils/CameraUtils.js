 ...
 *
 *   --A--
 *   setMultWindowOffset( fullWidth, fullHeight, w * 0, h * 0, w, h );
 *   --B--
 *   setMultWindowOffset( fullWidth, fullHeight, w * 1, h * 0, w, h );
 *   --C--
 *   setMultWindowOffset( fullWidth, fullHeight, w * 2, h * 0, w, h );
 *   --D--
 *   setMultWindowOffset( fullWidth, fullHeight, w * 0, h * 1, w, h );
 *   --E--
 *   setMultWindowOffset( fullWidth, fullHeight, w * 1, h * 1, w, h );
 *   --F--
 *   setMultWindowOffset( fullWidth, fullHeight, w * 2, h * 1, w, h );
 *
 *   Note there is no reason monitors have to be the same size or in a grid.
 */
	function setMultWindowOffset( camera, fullWidth, fullHeight, viewX, viewY, viewWidth, viewHeight ) {
		...
		const leftOffset = viewX * ( width / fullWidth );
		const rightOffset = ( left + leftOffset + width * ( viewWidth / fullWidth ) ) - right;
		const topOffset = - ( viewY * ( height / fullHeight ) );
		const bottomOffset = ( top + topOffset - height * ( viewHeight / fullHeight ) ) - bottom;

		camera.setViewOffset( rightOffset, leftOffset, topOffset, bottomOffset );

	}

	THREE.CameraUtils = {};
	THREE.CameraUtils.frameCorners = frameCorners;
	THREE.CameraUtils.setMultWindowOffset = setMultWindowOffset;
