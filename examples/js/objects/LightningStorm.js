	...
	class LightningStorm extends THREE.Object3D {
		constructor(stormParams = {}) {
			...
			for (let i = 0; i < this.stormParams.maxLightnings; i++) {
				const lightning = new THREE.LightningStrike(
		...
		update(time) {
			if (!this.inited) {
				this.nextLightningTime =
					this.getNextLightningTime(time) * Math.random();
				this.inited = true;
			}
		...
		copy(source, recursive) {
			...
			this.stormParams.lightningMinPeriod =
				source.stormParams.lightningMinPeriod;
			this.stormParams.lightningMaxPeriod =
				source.stormParams.lightningMaxPeriod;
			this.stormParams.lightningMinDuration =
