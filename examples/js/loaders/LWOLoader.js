(function () {
	...
	class MaterialParser {
		...
		parseMaterial(materialData, name, textures) {
			...
			if (materialType !== THREE.MeshPhongMaterial)
				delete params.refractionRatio; // PBR materials do not support "refractionRatio"
			...
			if (attributes.Luminous) {
				...
				if (attributes["Luminous THREE.Color"] && !maps.emissive) {
					params.emissive = new THREE.Color().fromArray(
						attributes["Luminous THREE.Color"].value
					);
				} else {
	...
	class GeometryParser {
		...
		parseMorphTargets(geometry, layer) {
			...
			for (const name in layer.morphTargets) {
				...
				geometry.morphAttributes.position[num] =
					new THREE.Float32BufferAttribute(remappedPoints, 3);
