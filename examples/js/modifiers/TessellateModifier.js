	...
	class TessellateModifier {
		...
		modify(geometry) {
			...
			while (tessellating && iteration < maxIterations) {
				...
				for (
					let i = 0, i2 = 0, il = positions.length;
					i < il;
					i += 9, i2 += 6
				) {
			...
			if (hasNormals) {
				geometry2.setAttribute(
					"normal",
					new THREE.Float32BufferAttribute(normals2, 3)
				);
			}
			...
			if (hasColors) {
				geometry2.setAttribute(
					"color",
					new THREE.Float32BufferAttribute(colors2, 3)
				);
			}
			...
			if (hasUV2s) {
				geometry2.setAttribute(
					"uv2",
					new THREE.Float32BufferAttribute(uv2s2, 2)
				);
			}
