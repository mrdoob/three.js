...
class EdgeSplitModifier {
  modify(geometry, cutOffAngle, tryKeepNormals = true) {
    ...
    const oldNbIndices = positions.length / geometry.getAttribute('position').itemSize;
    ...
    for (let i = 0; i < splitIndexes.length; i++) {
      ...
      for (const attribute of Object.values(newAttributes)) {
        for (let j = 0; j < attribute.itemSize; j++) {
          attribute.array[(oldNbIndices + i) * attribute.itemSize + j] = attribute.array[index * attribute.itemSize + j];
        }
      }
