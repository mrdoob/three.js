...
THREE.TAARenderPass.prototype.render = function ( renderer, writeBuffer, readBuffer, delta ) {
	...
	if( this.accumulateIndex >= 0 && this.accumulateIndex < jitterOffsets.length ) {
		...
		for ( var i = 0; i < numSamplesPerFrame; i ++ ) {
			...
			THREE.EffectRenderer.renderPass( renderer, this.materialComposite, this.sampleRenderTarget, ( this.accumulateIndex === 0 ) ? renderer.getClearColor() : null, ( this.accumulateIndex === 0 ) ? renderer.getClearAlpha() : null );
	...
	if( accumulationWeight > 0 ) {
		...
		THREE.EffectRenderer.renderPass( renderer, this.materialComposite, writeBuffer, renderer.getClearColor(), renderer.getClearAlpha() );
	}
	...
	if( accumulationWeight < 1.0 ) {
		...
		THREE.EffectRenderer.renderPass( renderer, this.materialComposite, writeBuffer, ( this.accumulateIndex === 0 ) ? renderer.getClearColor() : null, ( this.accumulateIndex === 0 ) ? renderer.getClearAlpha() : null );
	}
