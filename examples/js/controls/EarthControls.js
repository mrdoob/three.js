...
THREE.EarthControls = function ( object, domElement, earth ) {

    this.object = object;

    this.domElement = ( domElement !== undefined ) ? domElement : document;

    this.sphere = earth.sphere;
    this.radius = ( earth.radius !== undefined ) ? earth.radius : 6400;
    this.longitude = ( earth.lon !== undefined ) ? earth.lon : 0;
    this.latitude = ( earth.lat !== undefined ) ? earth.lat : 0;
    this.render = ( earth.render !== undefined ) ? earth.render : null;
    ...
    this.update = function () {
        ...
        return function update() {
            ...
            scope.sphere.rotation.set(
                scope.latitude * Math.PI / 180,
                (270 - scope.longitude) * Math.PI / 180,
                0
            );
            scope.render();
            // update condition is:
    ...
    var panWest = function( distance ) {
        var r = scope.radius + elevationShift;
        var latDelta = -Math.sin(spherical.theta) * (distance / 4000) * 180 / Math.PI;
        var lonDelta = Math.cos(spherical.theta) *
            (distance / (r * Math.cos(scope.latitude * Math.PI / 180))) * 180 / Math.PI;
  
        scope.longitude -= lonDelta;
          
        if (scope.latitude + latDelta < 80 && scope.latitude + latDelta > -80) {
            scope.latitude += latDelta;
        }
        // latitude = (latitude + 90) % 180 - 90;
        scope.longitude = (scope.longitude + 540) % 360 - 180;

    };

    var panNorth = function( distance ) {
        
        var r = scope.radius + elevationShift;

        var lonDelta = Math.sin(spherical.theta) *
              (distance / (r * Math.cos(scope.latitude * Math.PI / 180))) * 180 / Math.PI;
        ...
        scope.longitude = (scope.longitude + 360) % 360;
    ...
    var pan = function () {
        ...
        return function pan( deltaX, deltaY ) {
            ...
            if ( scope.object.isPerspectiveCamera ) {
                ...
                panWest( 2 * deltaX * targetDistance / element.clientHeight );
                panNorth( 2 * deltaY * targetDistance / element.clientHeight );
