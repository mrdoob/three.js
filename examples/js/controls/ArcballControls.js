		...
		constructor(_camera, domElement, scene = null) {
						...
						switch (mouseOp) {
							...
							case "FOV":
								if (this.camera.isPerspectiveCamera) {
									...
									const y =
										x *
										Math.tan(THREE.MathUtils.DEG2RAD * this.camera.fov * 0.5);

									//calculate new fov
									let newFov =
										THREE.MathUtils.RAD2DEG * (Math.atan(y / xNew) * 2);
									...
									const newDistance =
										y / Math.tan(THREE.MathUtils.DEG2RAD * (newFov / 2));
									size = x / newDistance;
									this.setFov(newFov);
									this.applyTransformMatrix(
						...
						case STATE.FOV:
							if (this.enableZoom && this.camera.isPerspectiveCamera) {
								if (restart) {
									...
									const y =
										x *
										Math.tan(THREE.MathUtils.DEG2RAD * this._fovState * 0.5);
			...
			this.onRotateMove = () => {
				if (this.enabled && this.enableRotate) {
					...
					this._currentFingerRotation =
						...
						this.getAngle(this._touchStart[1], this._touchStart[0]);
					if (!this.enablePan) {
						rotationPoint = new THREE.Vector3().setFromMatrixPosition(
			...
			this.onTriplePanMove = () => {
				if (this.enabled && this.enableZoom) {
					...
					const y =
						x * Math.tan(THREE.MathUtils.DEG2RAD * this._fovState * 0.5);
