...
class OrbitControls extends THREE.EventDispatcher {
  constructor(object, domElement) {
    ...
    this.update = function () {
      ...
      return function update() {
        ...
        if (zoomChanged || lastPosition.distanceToSquared(scope.object.position) > EPS || 8 * (1 - lastQuaternion.dot(scope.object.quaternion)) > EPS) {
          ...
          // ZOOM-TO-CURSOR
          if (scope.zoomToCursor && scope.adjustmentAfterZoomNeeded) {
    ...
    scope.domElement.addEventListener('mousemove', event => {
      ...
      scope.cursorScreen.copy(new THREE.Vector3(event.clientX / scope.domElement.clientWidth * 2 - 1, -(event.clientY / scope.domElement.clientHeight) * 2 + 1, scope.target.clone().project(scope.object).z));
    });
    const handleTouch = event => {
      ...
      if (touch !== undefined) {
        scope.cursorScreen.copy(new THREE.Vector3(touch.x / scope.domElement.clientWidth * 2 - 1, -(touch.y / scope.domElement.clientHeight) * 2 + 1, scope.target.clone().project(scope.object).z));
      }
    };
