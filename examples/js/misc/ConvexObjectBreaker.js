			...
			function subdivideRadial(subObject, startAngle, endAngle, numIterations) {
				...
				if (numIterations === 0) {
					...
					if (numIterations <= maxRadialIterations) {
						angle =
							(endAngle - startAngle) * (0.2 + 0.6 * Math.random()) +
							startAngle;
		...
		cutByPlane(object, plane, output) {
			...
			for (let i = 0; i < numFaces; i++) {
				...
				for (let segment = 0; segment < 3; segment++) {
					...
					if ((mark0 === 1 && mark1 === 2) || (mark0 === 2 && mark1 === 1)) {
						...
						intersection = localPlane.intersectLine(
							this.tempLine1,
							intersection
						);
						if (intersection === null) {
							// Shouldn't happen
							console.error(
								"Internal error: segment does not intersect plane."
							);
			...
			if (numPoints1 > 4) {
				object1 = new THREE.Mesh(
					new THREE.ConvexGeometry(points1),
					object.material
				);
			...
			if (numPoints2 > 4) {
				object2 = new THREE.Mesh(
					new THREE.ConvexGeometry(points2),
					object.material
				);
