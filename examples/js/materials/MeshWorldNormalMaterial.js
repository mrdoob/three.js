( function () {
	...
	class MeshWorldNormalMaterial extends THREE.ShaderMaterial {
		...
		constructor( parameters ) {
			...
			parameters.fragmentShader = 'uniform mat4 viewMatrixInverse;' + '\n' + THREE.ShaderLib.normal.fragmentShader.replace( 'gl_FragColor = ', 'normal = normalize(mat3(viewMatrixInverse) * normal);' + '\n' + 'gl_FragColor = ' );
			super( parameters );

			this.updateMeshOnBeforeRender = function ( mesh ) {
				...
				mesh.onBeforeRender = function ( renderer, scene, camera, geometry, material, group ) {
					...
					if ( this.material.isMeshWorldNormalMaterial ) this.material.uniforms.viewMatrixInverse.value.copy( camera.matrixWorld );
