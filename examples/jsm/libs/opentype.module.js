...
function Table(tableName, fields, options) {
	...
	if (
		fields.length &&
		(fields[0].name !== "coverageFormat" || fields[0].value === 1)
	) {
...
Glyph.prototype.bindConstructorValues = function (options) {
	...
	this.unicodes =
		options.unicodes || options.unicode !== undefined ? [options.unicode] : [];
...
Glyph.prototype.getMetrics = function () {
	...
	metrics.rightSideBearing =
		this.advanceWidth - metrics.leftSideBearing - (metrics.xMax - metrics.xMin);
	return metrics;
};
...
function makeColrTable(ref) {
	...
	if (version === void 0) version = 0x0000;
...
function makeCpalTable(ref) {
	...
	check.argument(
		!numPaletteEntries && colorRecordIndices.length == 1,
	...
	return new table.Table(
		...
		[
			{ name: "version", type: "USHORT", value: version },
			{
				...
				type: "USHORT",
				value: numPaletteEntries || colorRecords.length,
			},
			...
			{ name: "numColorRecords", type: "USHORT", value: colorRecords.length },
			{
				...
				type: "ULONG",
				value: 12 + 2 * colorRecordIndices.length,
			},
		].concat(
			colorRecordIndices.map(function (palette, i) {
				return {
					name: "colorRecordIndices_" + i,
					type: "USHORT",
					value: palette,
				};
			}),
...
Tokenizer.prototype.registerModifier = function (
	...
	this.events.newToken.subscribe(function (token, contextParams) {
		...
		var canApplyModifier =
			condition === null || condition.apply(this, conditionParams) === true;
...
function chainingSubstitutionFormat3(contextParams, subtable) {
	...
	while (
		lookaheadContext.length &&
		isTashkeelArabicChar(lookaheadContext[0].char)
	) {
	...
	while (
		backtrackContext.length &&
		isTashkeelArabicChar(backtrackContext[0].char)
	) {
	...
	if (contextRulesMatch) {
		for (var i = 0; i < subtable.lookupRecords.length; i++) {
			...
			for (var s = 0; s < lookupTable.subtables.length; s++) {
				...
				var substitutionType = this.getSubstitutionType(
					lookupTable,
					subtable$1
				);
