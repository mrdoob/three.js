...
var k = R((exports, module) => {
	...
	function map(t, e) {
		for (var r = [], n = 0; n < t.length; n++) r.push(e.call(null, t[n], n));
		return r;
	}
	exports.map = map;
	function flatten(t) {
	...
	function forEach(t, e) {
		if (Array.isArray(t))
			for (var r = 0; r < t.length; r++) e.call(null, t[r], r);
		else if (isObject(t))
			for (var n = keys(t), r = 0; r < n.length; r++) {
				var i = n[r],
					a = t[i];
				e.call(null, a, i);
			}
	...
	function every(t, e) {
		for (var r = 0; r < t.length; r++) if (!e(t[r], r)) return !1;
		return !0;
	}
	exports.every = every;
	...
	function zipObject(t, e) {
		...
		for (var r = {}, n = 0; n < t.length; n++) r[t[n]] = e[n];
		return r;
	}
		...
		(t.prototype.saveState = function () {
			...
			(t.prototype.pattern = function (u) {
				...
				for (
					var A = {
						type: "Flags",
						loc: { begin: this.idx, end: u.length },
						global: !1,
						ignoreCase: !1,
						multiLine: !1,
						unicode: !1,
				...
				return { type: "Pattern", flags: A, value: d, loc: this.loc(0) };
			}),
			...
			(t.prototype.alternative = function () {
				for (var u = [], d = this.idx; this.isTerm(); ) u.push(this.term());
			...
			(t.prototype.assertion = function () {
				...
				switch (this.popChar()) {
					...
					case "(":
						...
						return (
							this.consumeChar(")"), { type: d, value: A, loc: this.loc(u) }
						);
				}
				c();
			}),
					...
					case "{":
						...
						switch (this.popChar()) {
							...
							case ",":
								var g;
								this.isDigit()
									? ((g = this.integerIncludingZero()),
									  (d = { atLeast: _, atMost: g }))
									: (d = { atLeast: _, atMost: Infinity }),
									this.consumeChar("}");
								break;
						}
			...
			(t.prototype.atom = function () {
				...
				return (
					u === void 0 &&
						...
						(u = this.patternCharacter()),
					s(u),
					(u.loc = this.loc(d)),
					this.isQuantifier() && (u.quantifier = this.quantifier()),
					u
				);
			}),
...
`:
			...
			(t.prototype.characterClass = function () {
				...
				for (
					this.consumeChar("["),
						this.peekChar(0) === "^" && (this.consumeChar("^"), (d = !0));
					this.isClassAtom();
					...
					if (_ && this.isRangeDash()) {
						...
						if (y) {
							...
							u.push({ from: A.value, to: g.value });
...
`:
		...
		var f,
			p = [];
		for (f = i("0"); f <= i("9"); f++) p.push(f);
		var l = [i("_")].concat(p);
		for (f = i("a"); f <= i("z"); f++) l.push(f);
		for (f = i("A"); f <= i("Z"); f++) l.push(f);
		var m = [
			i(" "),
			i("\f"),
...
`),
		...
		function v() {}
		return (
			(v.prototype.visitChildren = function (u) {
				for (var d in u) {
					var A = u[d];
					u.hasOwnProperty(d) &&
						(A.type !== void 0
			...
			{ RegExpParser: t, BaseRegExpVisitor: v, VERSION: "0.5.0" }
		);
	});
});
var Lt = R((He) => {
...
var pn = R((re) => {
	...
	re.canMatchCharCode =
		re.firstCharOptimizedIndices =
		re.getOptimizedStartCodesIndices =
		re.failedOptimizationPrefixMsg =
			void 0;
	var un = xt(),
		pe = k(),
		cn = Lt(),
...
`) +
	...
	function Mt(t, e, r) {
		switch (t.type) {
			case "Disjunction":
				for (var n = 0; n < t.value.length; n++) Mt(t.value[n], e, r);
				break;
			case "Alternative":
					...
					switch (o.type) {
						...
						case "Set":
							...
							pe.forEach(o.value, function (f) {
								if (typeof f == "number") bt(f, e, r);
								else {
									var p = f;
									if (r === !0)
										for (var l = p.from; l <= p.to; l++) bt(l, e, r);
									else {
										for (
											var l = p.from;
											l <= p.to && l < Ce.minOptimizationVal;
											l++
										)
											bt(l, e, r);
										if (p.to >= Ce.minOptimizationVal)
											for (
												var m =
														p.from >= Ce.minOptimizationVal
															? p.from
															: Ce.minOptimizationVal,
													v = p.to,
													u = Ce.charCodeToOptimizedIndex(m),
													d = Ce.charCodeToOptimizedIndex(v),
													A = u;
												A <= d;
												A++
											)
	...
	function yr(t) {
		return t.quantifier && t.quantifier.atLeast === 0
			? !0
			: t.value
			? pe.isArray(t.value)
				? pe.every(t.value, yr)
	...
	var za = (function (t) {
		...
		return (
			(e.prototype.visitChildren = function (r) {
				if (this.found !== !0) {
					switch (r.type) {
						case "Lookahead":
			...
			(e.prototype.visitSet = function (r) {
				r.complement
					? fn(r, this.targetCharCodes) === void 0 && (this.found = !0)
					: fn(r, this.targetCharCodes) !== void 0 && (this.found = !0);
			}),
			e
		);
...
var Tr = R((T) => {
	...
	T.charCodeToOptimizedIndex =
		T.minOptimizationVal =
		T.buildLineBreakIssueMessage =
		T.LineTerminatorOptimizedTester =
		T.isShortPattern =
		T.isCustomPattern =
		T.cloneEmptyGroups =
		T.performWarningRuntimeChecks =
		T.performRuntimeChecks =
		T.addStickyFlag =
		T.addStartOfInput =
		T.findUnreachablePatterns =
		T.findModesThatDoNotExist =
		T.findInvalidGroupType =
		T.findDuplicatePatterns =
		T.findUnsupportedFlags =
		T.findStartOfInputAnchor =
		T.findEmptyMatchRegExps =
		T.findEndOfInputAnchor =
		T.findInvalidPatterns =
		T.findMissingPatterns =
		T.validatePatterns =
		T.analyzeTokenTypes =
		T.enableSticky =
		T.disableSticky =
			...
			(i = !1),
				(a = h.map(n, function (g) {
					...
					if (h.isRegExp(y)) {
						...
						if (h.isFunction(y)) return (i = !0), { exec: y };
								...
								if (y.PATTERN.unicode)
									(A = !1),
										e.ensureOptimizations &&
											h.PRINT_ERROR(
												"" +
													Ye.failedOptimizationPrefixMsg +
													("	Unable to analyze < " +
														y.PATTERN.toString() +
														` > pattern.
`) +
	...
	For details See: https://chevrotain.io/docs/guide/resolving_lexer_errors.html#UNICODE_OPTIMIZE`
									...
									var Z = Ye.getOptimizedStartCodesIndices(
										y.PATTERN,
										e.ensureOptimizations
									);
									h.isEmpty(Z) && (A = !1),
										h.forEach(Z, function (ue) {
											Rr(g, ue, d[b]);
										});
								}
							else
			...
			r = h.reduce(
				...
				function (n, i, a) {
					...
					return (
						o === F.Lexer.NA ||
							(h.isString(o)
								? n.push({ str: o, idx: a, tokenType: i })
								: h.isRegExp(o) &&
								  no(o) &&
								  n.push({ str: o.source, idx: a, tokenType: i })),
						n
					);
				},
				[]
...
` +
							("Because it appears AFTER the Token Type ->" + n.name + "<-") +
...
`,
					type: F.LexerDefinitionErrorType
...
`,
			...
			h.has(t, T.MODES) &&
				h.forEach(t.modes, function (i, a) {
...
`),
	...
	function ao(t, e, r) {
		var n = [],
			...
			a = h.compact(
				h.flatten(
					h.mapValues(t.modes, function (c) {
						return c;
					})
				)
			),
		...
		return (
			e &&
				h.forEach(o, function (c) {
					var f = En(c, s);
					if (f !== !1) {
						var p = Sn(c, f),
							l = { message: p, type: f.issue, tokenType: c };
						n.push(l);
	...
	function En(t, e) {
		...
		if (h.isRegExp(t.PATTERN)) {
			...
			if (Ar(t)) return { issue: F.LexerDefinitionErrorType.CUSTOM_LINE_BREAK };
			throw Error("non exhaustive match");
		}
	}
	function Sn(t, e) {
		if (e.issue === F.LexerDefinitionErrorType.IDENTIFY_TERMINATOR)
			return (
...
`) +
		...
		if (e.issue === F.LexerDefinitionErrorType.CUSTOM_LINE_BREAK)
			return (
...
`) +
	...
	function $a() {
		if (h.isEmpty(Ft)) {
			Ft = new Array(65536);
			for (var t = 0; t < 65536; t++) Ft[t] = t > 255 ? 255 + ~~(t / 255) : t;
		}
	}
});
var Xe = R((N) => {
	...
	N.isTokenType =
		N.hasExtendingTokensTypesMapProperty =
		N.hasExtendingTokensTypesProperty =
		N.hasCategoriesProperty =
		N.hasShortKeyProperty =
		N.singleAssignCategoriesToksMap =
		N.assignCategoriesMapProp =
		N.assignCategoriesTokensProp =
		N.assignTokenDefaultProps =
		N.expandCategories =
		N.augmentTokenTypes =
		N.tokenIdxToClass =
		N.tokenShortNameIdx =
		N.tokenStructuredMatcherNoCategories =
		N.tokenStructuredMatcher =
			void 0;
	var V = k();
	function so(t, e) {
	...
	function Cn(t) {
		V.forEach(t, function (e) {
			bn(e) ||
				((N.tokenIdxToClass[N.tokenShortNameIdx] = e),
				(e.tokenTypeIdx = N.tokenShortNameIdx++)),
				Nr(e) && !V.isArray(e.CATEGORIES) && (e.CATEGORIES = [e.CATEGORIES]),
				Nr(e) || (e.CATEGORIES = []),
				Fn(e) || (e.categoryMatches = []),
				wn(e) || (e.categoryMatchesMap = {});
		});
	}
	...
	function Ln(t) {
		V.forEach(t, function (e) {
			(e.categoryMatches = []),
				V.forEach(e.categoryMatchesMap, function (r, n) {
...
a boolean 2nd argument is no longer supported`);
			...
			i === !0
				? ((this.traceInitMaxIdent = Infinity), (this.traceInitPerf = !0))
				: typeof i == "number" &&
				  ((this.traceInitMaxIdent = i), (this.traceInitPerf = !0)),
				(this.traceInitIndent = -1),
				this.TRACE_INIT("Lexer Constructor", function () {
					var a,
						o = !0;
					n.TRACE_INIT("Lexer Config handling", function () {
						if (n.config.lineTerminatorsPattern === pt.lineTerminatorsPattern)
							n.config.lineTerminatorsPattern =
								Ee.LineTerminatorOptimizedTester;
						else if (
							n.config.lineTerminatorCharacters === pt.lineTerminatorCharacters
						)
	...
	For details See: https://chevrotain.io/docs/guide/resolving_lexer_errors.html#MISSING_LINE_TERM_CHARS`);
						...
						(n.trackStartLines = /full|onlyStart/i.test(
							...
							w.isArray(e)
								? ((a = { modes: {} }),
								  (a.modes[Ee.DEFAULT_MODE] = w.cloneArr(e)),
								  (a[Ee.DEFAULT_MODE] = Ee.DEFAULT_MODE))
								: ((o = !1), (a = w.cloneObj(e)));
					}),
						n.config.skipValidations === !1 &&
						...
						w.forEach(a.modes, function (p, l) {
							...
							n.TRACE_INIT("Mode: <" + l + "> processing", function () {
								if (
									...
									n.TRACE_INIT("analyzeTokenTypes", function () {
										...
										(n.patternIdxToConfig[l] = m.patternIdxToConfig),
										(n.charCodeToPatternIdxToConfig[l] =
											m.charCodeToPatternIdxToConfig),
										(n.emptyGroups = w.merge(n.emptyGroups, m.emptyGroups)),
										(n.hasCustom = m.hasCustom || n.hasCustom),
										(n.canModeBeOptimized[l] = m.canBeOptimized);
								}
							});
						}),
						(n.defaultMode = a.defaultMode),
						!w.isEmpty(n.lexerDefinitionErrors) &&
...
` + f
					...
					w.forEach(n.lexerDefinitionWarning, function (p) {
						...
						n.TRACE_INIT("Choosing sub-methods implementations", function () {
							if (
								(Ee.SUPPORT_STICKY
									? ((n.chopInput = w.IDENTITY), (n.match = n.matchWithTest))
									: ((n.updateLastIndex = w.NOOP), (n.match = n.matchWithExec)),
								o && (n.handleModes = w.NOOP),
								n.trackStartLines === !1 && (n.computeNewColumn = w.IDENTITY),
								n.trackEndLines === !1 &&
									(n.updateTokenEndLineColumnLocation = w.NOOP),
						...
						n.TRACE_INIT("Failed Optimization Warnings", function () {
							...
							if (r.ensureOptimizations && !w.isEmpty(p))
								throw Error(
									"Lexer Modes: < " +
										p.join(", ") +
										` > cannot be optimized.
	 ...
	 Or inspect the console log for details on how to resolve these issues.`
		...
		return (
			(t.prototype.tokenize = function (e, r) {
				if (
					(r === void 0 && (r = this.defaultMode),
					!w.isEmpty(this.lexerDefinitionErrors))
				) {
...
` + i
			...
			(t.prototype.tokenizeInternal = function (e, r) {
				...
				var wa = function (J) {
					if (It.length === 1 && J.tokenType.PUSH_MODE === void 0) {
						var lt =
							n.config.errorMessageProvider.buildUnableToPopLexerModeMessage(J);
						ue.push({
						...
						It.pop();
						var ze = w.last(It);
						(we = n.patternIdxToConfig[ze]),
							(ot = n.charCodeToPatternIdxToConfig[ze]),
							(Ke = we.length);
				...
				function tn(J) {
					It.push(J),
						(ot = this.charCodeToPatternIdxToConfig[J]),
						(we = this.patternIdxToConfig[J]),
						(Ke = we.length),
						(Ke = we.length);
				...
				for (var me; L < b; ) {
					...
					for (i = 0; i < Da; i++) {
						...
						if (
							(ut !== !1
								...
								? ((g = De.exec(y, L, Z, xe)),
								  g !== null
										? ((c = g[0]), g.payload !== void 0 && (f = g.payload))
										: (c = null))
								: (this.updateLastIndex(De, L), (c = this.match(De, e, L))),
							c !== null)
						) {
							if (((s = me.longerAlt), s !== void 0)) {
								...
								(p = null),
									fr.isCustom === !0
										? ((g = pr.exec(y, L, Z, xe)),
										  g !== null
												? ((o = g[0]), g.payload !== void 0 && (p = g.payload))
												: (o = null))
										: (this.updateLastIndex(pr, L), (o = this.match(pr, e, L))),
									o && o.length > c.length && ((c = o), (f = p), (me = fr));
							}
							break;
						}
					}
					if (c !== null) {
						if (
							...
							m !== void 0 &&
								((v = me.tokenTypeIdx),
								(u = this.createTokenInstance(c, L, v, me.tokenType, Q, te, l)),
								this.handlePayload(u, f),
								m === !1 ? (se = this.addToken(Z, se, u)) : xe[m].push(u)),
							(e = this.chopInput(e, l)),
							(L = L + l),
							(te = this.computeNewColumn(te, l)),
							it === !0 && me.canLineTerminator === !0)
						) {
							var kt = 0,
								hr = void 0,
								dr = void 0;
							...
							kt !== 0 &&
								((Q = Q + kt),
								(te = l - dr),
								this.updateTokenEndLineColumnLocation(u, m, dr, kt, Q, te, l));
						}
						this.handleModes(me, wa, tn, u);
					} else {
						for (var vr = L, an = Q, on = te, ct = !1; !ct && L < b; )
							for (
								A = y.charCodeAt(L), e = this.chopInput(e, 1), L++, a = 0;
								a < Ke;
								a++
							) {
								var mr = we[a],
								...
								if (
									(ut !== !1
										? y.charCodeAt(L) === ut && (ct = !0)
										: mr.isCustom === !0
										? (ct = De.exec(y, L, Z, xe) !== null)
										: (this.updateLastIndex(De, L), (ct = De.exec(e) !== null)),
									ct === !0)
								)
									break;
							}
				...
				return (
					this.hasCustom || (Z.length = se),
					{ tokens: Z, groups: xe, errors: ue }
				);
			}),
			...
			(t.prototype.updateTokenEndLineColumnLocation = function (
				e,
				r,
				n,
				i,
				...
				r !== void 0 &&
					...
					(f = c ? -1 : 0),
					(i === 1 && c === !0) ||
						((e.endLine = a + f), (e.endColumn = o - 1 + -f)));
			}),
			...
			(t.prototype.handlePayload = function (e, r) {}),
...
var Ue = R((H) => {
	...
	function go(t) {
		var e = t.pattern,
			r = {};
		if (
			((r.name = t.name), Te.isUndefined(e) || (r.PATTERN = e), Te.has(t, _o))
		)
			throw `The parent property is no longer supported.
See: https://github.com/chevrotain/chevrotain/issues/564#issuecomment-349062346 for details.`;
		return (
			Te.has(t, Un) && (r.CATEGORIES = t[Un]),
			Pr.augmentTokenTypes([r]),
			Te.has(t, Gn) && (r.LABEL = t[Gn]),
			Te.has(t, Wn) && (r.GROUP = t[Wn]),
			Te.has(t, qn) && (r.POP_MODE = t[qn]),
			Te.has(t, Bn) && (r.PUSH_MODE = t[Bn]),
			Te.has(t, jn) && (r.LONGER_ALT = t[jn]),
			Te.has(t, Vn) && (r.LINE_BREAKS = t[Vn]),
			Te.has(t, Kn) && (r.START_CHARS_HINT = t[Kn]),
			r
		);
	}
...
var ne = R((S) => {
	...
	S.serializeProduction =
		S.serializeGrammar =
		S.Terminal =
		S.Alternation =
		S.RepetitionWithSeparator =
		S.Repetition =
		S.RepetitionMandatoryWithSeparator =
		S.RepetitionMandatory =
		S.Option =
		S.Alternative =
		S.Rule =
		S.NonTerminal =
		S.AbstractProduction =
			void 0;
	var G = k(),
		Oo = Ue(),
		Re = (function () {
			...
			return (
				...
				(t.prototype.accept = function (e) {
					e.visit(this),
						G.forEach(this.definition, function (r) {
							r.accept(e);
						});
				}),
				t
	...
	function ht(t) {
		...
		if (t instanceof Hn)
			return { type: "NonTerminal", name: t.nonTerminalName, idx: t.idx };
		if (t instanceof Xn)
		...
		if (t instanceof Qn)
			return {
				...
				idx: t.idx,
				separator: ht(new Dt({ terminalType: t.separator })),
				definition: e(t.definition),
			};
		if (t instanceof ei)
			return {
		...
		if (t instanceof Dt) {
			...
			return (
				t.terminalType.PATTERN && (r.pattern = G.isRegExp(n) ? n.source : n), r
			);
		} else {
			if (t instanceof Yn)
				return {
...
var Gt = R((Ut) => {
	...
	var Sr = k(),
		...
		Io = (function () {
			function t() {}
			return (
				(t.prototype.walk = function (e, r) {
					var n = this;
					r === void 0 && (r = []),
						Sr.forEach(e.definition, function (i, a) {
							var o = Sr.drop(e.definition, a + 1);
							if (i instanceof ie.NonTerminal) n.walkProdRef(i, o, r);
							else if (i instanceof ie.Terminal) n.walkTerminal(i, o, r);
							else if (i instanceof ie.Alternative) n.walkFlat(i, o, r);
							else if (i instanceof ie.Option) n.walkOption(i, o, r);
							else if (i instanceof ie.RepetitionMandatory)
								n.walkAtLeastOne(i, o, r);
							else if (i instanceof ie.RepetitionMandatoryWithSeparator)
								n.walkAtLeastOneSep(i, o, r);
							else if (i instanceof ie.RepetitionWithSeparator)
								n.walkManySep(i, o, r);
							else if (i instanceof ie.Repetition) n.walkMany(i, o, r);
							else if (i instanceof ie.Alternation) n.walkOr(i, o, r);
							else throw Error("non exhaustive match");
						});
				}),
				...
				(t.prototype.walkAtLeastOne = function (e, r, n) {
					var i = [new ie.Option({ definition: e.definition })].concat(r, n);
					this.walk(e, i);
				}),
	...
	function ri(t, e, r) {
		var n = [
				new ie.Option({
					definition: [new ie.Terminal({ terminalType: t.separator })].concat(
						t.definition
					),
				}),
			],
...
var vt = R((X) => {
	...
	X.collectMethods =
		X.DslMethodsCollectorVisitor =
		X.getProductionDslName =
		X.isBranchingProd =
		X.isOptionalProd =
		X.isSequenceProd =
			void 0;
	...
	function xo(t) {
		return (
			t instanceof W.Alternative ||
			t instanceof W.Option ||
			t instanceof W.Repetition ||
			t instanceof W.RepetitionMandatory ||
			t instanceof W.RepetitionMandatoryWithSeparator ||
			t instanceof W.RepetitionWithSeparator ||
			t instanceof W.Terminal ||
			t instanceof W.Rule
		);
	}
	X.isSequenceProd = xo;
	function xr(t, e) {
		...
		return r
			? !0
			: t instanceof W.Alternation
			? dt.some(t.definition, function (n) {
					return xr(n, e);
			  })
			: t instanceof W.NonTerminal && dt.contains(e, t)
			? !1
			: t instanceof W.AbstractProduction
			? (t instanceof W.NonTerminal && e.push(t),
			  dt.every(t.definition, function (n) {
					return xr(n, e);
			  }))
			: !1;
	}
	...
	function Lo(t) {
		...
		if (t instanceof W.RepetitionMandatoryWithSeparator)
	...
	var ni = (function (t) {
		...
		return (
			...
			(e.prototype.visitTerminal = function (r) {
				...
				dt.has(this.dslMethods, n) || (this.dslMethods[n] = []),
					this.dslMethods[n].push(r);
			}),
...
var Lr = R((Ne) => {
	...
	function ai(t) {
		for (
			var e = [], r = t.definition, n = 0, i = r.length > n, a, o = !0;
			i && o;

		)
			(a = r[n]),
				(o = Cr.isOptionalProd(a)),
				(e = e.concat(jt(a))),
				(n = n + 1),
				(i = r.length > n);
		return qt.uniq(e);
...
var pi = R((he) => {
	...
	Object.defineProperty(he, "__esModule", { value: !0 });
	he.buildInProdFollowPrefix =
		he.buildBetweenProdsFollowPrefix =
		he.computeAllProdsFollows =
		he.ResyncFollowsWalker =
			void 0;
	var Fo = Gt(),
		wo = Lr(),
		ui = k(),
			...
			return (
				...
				(e.prototype.walkProdRef = function (r, n, i) {
					var a = li(r.referencedRule, r.idx) + this.topProd.name,
						o = n.concat(i),
						s = new Do.Alternative({ definition: o }),
						c = wo.first(s);
					this.follows[a] = c;
				}),
				e
...
inside top level rule: ->` +
	...
	Me.defaultGrammarValidatorErrorProvider = {
		buildDuplicateFoundError: function (t, e) {
			function r(p) {
				return p instanceof br.Terminal
					? p.terminalType.name
					: p instanceof br.NonTerminal
					? p.nonTerminalName
					: "";
			}
			var n = t.name,
...
` +
				("<OR" +
					...
					`> Rule.
 has ` +
					(t.alternation.definition.length + 1) +
					" alternatives.");
			return e;
		},
...
see: https://en.wikipedia.org/wiki/LL_parser#Left_Factoring.`;
		...
		buildDuplicateRuleNameError: function (t) {
			var e;
			t.topLevelRule instanceof br.Rule
	...
	var vi = (function (t) {
		...
		return (
			...
			(e.prototype.visitNonTerminal = function (r) {
				...
				if (n) r.referencedRule = n;
				else {
					var i = this.errMsgProvider.buildRuleNotFoundError(
						this.currTopLevel,
						r
					);
					this.errors.push({
...
var Tt = R((j) => {
	...
	j.nextPossibleTokensAfter =
		j.possiblePathsFrom =
		j.NextTerminalAfterAtLeastOneSepWalker =
		j.NextTerminalAfterAtLeastOneWalker =
		j.NextTerminalAfterManySepWalker =
		j.NextTerminalAfterManyWalker =
		j.AbstractNextTerminalAfterProductionWalker =
		j.NextAfterTokenWalker =
		j.AbstractNextPossibleTokensWalker =
			void 0;
	var Ei = Gt(),
		I = k(),
		Ko = Lr(),
		...
		Ti = (function (t) {
			...
			return (
				(e.prototype.startWalking = function () {
					...
					return (
						(this.ruleStack = I.cloneArr(this.path.ruleStack).reverse()),
						(this.occurrenceStack = I.cloneArr(
				...
				(e.prototype.walk = function (r, n) {
					n === void 0 && (n = []),
						this.found || t.prototype.walk.call(this, r, n);
				}),
				(e.prototype.walkProdRef = function (r, n, i) {
					if (
						r.referencedRule.name === this.nextProductionName &&
						r.idx === this.nextProductionOccurrence
					) {
						var a = n.concat(i);
	...
	var zo = (function (t) {
		...
		return (
			(e.prototype.walkTerminal = function (r, n, i) {
				if (
					this.isAtEndOfPath &&
					r.terminalType.name === this.nextTerminalName &&
					r.idx === this.nextTerminalOccurrence &&
					!this.found
				) {
					var a = n.concat(i),
						o = new O.Alternative({ definition: a });
	...
	function yi(t, e, r) {
		...
		for (; r.length < e && i < t.length; ) {
			...
			if (s instanceof O.Option) n = o(s.definition);
			else if (s instanceof O.RepetitionMandatory) {
				var c = s.definition.concat([
					new O.Repetition({ definition: s.definition }),
				]);
				return o(c);
			} else if (s instanceof O.RepetitionMandatoryWithSeparator) {
				var c = [
					new O.Alternative({ definition: s.definition }),
					new O.Repetition({
						definition: [new O.Terminal({ terminalType: s.separator })].concat(
							s.definition
						),
					}),
				];
				...
				if (s instanceof O.Alternation)
					return (
						I.forEach(s.definition, function (f) {
							I.isEmpty(f.definition) === !1 && (n = o(f.definition));
						}),
						n
					);
				if (s instanceof O.Terminal) r.push(s.terminalType);
				else throw Error("non exhaustive match");
			}
			i++;
		}
		return n.push({ partialPath: r, suffixDef: I.drop(t, i) }), n;
	}
	...
	function Qo(t, e, r, n) {
		var i = "EXIT_NONE_TERMINAL",
			...
			l = [];
		for (
			l.push({ idx: -1, def: t, ruleStack: [], occurrenceStack: [] });
			!I.isEmpty(l);
			...
			if (!I.isEmpty(v)) {
				...
				if (_ === i) {
					...
					var fe = { idx: u, def: I.drop(v), ruleStack: d, occurrenceStack: A };
					l.push(fe), l.push(o);
					var Z = {
						idx: u,
					...
					var ue = new O.Repetition({ definition: _.definition, idx: _.idx }),
						Q = _.definition.concat([ue], I.drop(v)),
						g = { idx: u, def: Q, ruleStack: d, occurrenceStack: A };
					l.push(g);
				} else if (_ instanceof O.RepetitionMandatoryWithSeparator) {
					var te = new O.Terminal({ terminalType: _.separator }),
						ue = new O.Repetition({
					...
					var te = new O.Terminal({ terminalType: _.separator }),
						...
						Z = { idx: u, def: Q, ruleStack: d, occurrenceStack: A };
					l.push(Z);
				} else if (_ instanceof O.Repetition) {
					var fe = { idx: u, def: I.drop(v), ruleStack: d, occurrenceStack: A };
					l.push(fe), l.push(o);
					var xe = new O.Repetition({ definition: _.definition, idx: _.idx }),
						Q = _.definition.concat([xe], I.drop(v)),
						Z = { idx: u, def: Q, ruleStack: d, occurrenceStack: A };
					l.push(Z);
				...
				else if (_ instanceof O.Rule) l.push(Zo(_, u, d, A));
				else throw Error("non exhaustive match");
			}
		}
		return p;
	...
	function Zo(t, e, r, n) {
		...
		var a = I.cloneArr(n);
		return (
			a.push(1), { idx: e, def: t.definition, ruleStack: i, occurrenceStack: a }
		);
	}
});
var yt = R((C) => {
	...
	C.areTokenCategoriesNotUsed =
		C.isStrictPrefixOfPath =
		C.containsPath =
		C.getLookaheadPathsForOptionalProd =
		C.getLookaheadPathsForOr =
		C.lookAheadSequenceFromAlternatives =
		C.buildSingleAlternativeLookaheadFunction =
		C.buildAlternativesLookAheadFunc =
		C.buildLookaheadFuncForOptionalProd =
		C.buildLookaheadFuncForOr =
		C.getProdType =
		C.PROD_TYPE =
			void 0;
	var D = k(),
	...
	function ts(t) {
		...
		if (t instanceof We.RepetitionMandatoryWithSeparator)
			return z.REPETITION_MANDATORY_WITH_SEPARATOR;
		if (t instanceof We.RepetitionWithSeparator)
	...
	function rs(t, e, r, n, i, a) {
		var o = Ai(t, e, r),
			s = Fr(o)
				? Kt.tokenStructuredMatcherNoCategories
				: Kt.tokenStructuredMatcher;
		return a(o, n, s, i);
	}
		...
		if (a && !n) {
			var o = D.map(t, function (c) {
				...
				s = D.reduce(
					...
					function (c, f, p) {
						return (
							D.forEach(f, function (l) {
								D.has(c, l.tokenTypeIdx) || (c[l.tokenTypeIdx] = p),
									D.forEach(l.categoryMatches, function (m) {
										D.has(c, m) || (c[m] = p);
									});
							}),
							c
			...
			if (a.length === 1 && D.isEmpty(a[0].categoryMatches)) {
				...
				var c = D.reduce(
					...
					function (f, p, l) {
						return (
							(f[p.tokenTypeIdx] = !0),
							D.forEach(p.categoryMatches, function (m) {
								f[m] = !0;
							}),
							f
						);
	...
	var os = (function (t) {
			...
			function e(r, n, i) {
				...
				return (
					(a.topProd = r), (a.targetOccurrence = n), (a.targetProdType = i), a
				);
			}
			return (
				(e.prototype.startWalking = function () {
				...
				(e.prototype.checkIsTarget = function (r, n, i, a) {
					return r.idx === this.targetOccurrence && this.targetProdType === n
						? ((this.restDef = i.concat(a)), !0)
						: !1;
				}),
				(e.prototype.walkOption = function (r, n, i) {
					this.checkIsTarget(r, z.OPTION, n, i) ||
						t.prototype.walkOption.call(this, r, n, i);
				}),
				(e.prototype.walkAtLeastOne = function (r, n, i) {
					this.checkIsTarget(r, z.REPETITION_MANDATORY, n, i) ||
						t.prototype.walkOption.call(this, r, n, i);
				}),
				(e.prototype.walkAtLeastOneSep = function (r, n, i) {
					this.checkIsTarget(r, z.REPETITION_MANDATORY_WITH_SEPARATOR, n, i) ||
						t.prototype.walkOption.call(this, r, n, i);
				}),
				(e.prototype.walkMany = function (r, n, i) {
					this.checkIsTarget(r, z.REPETITION, n, i) ||
						t.prototype.walkOption.call(this, r, n, i);
				}),
				(e.prototype.walkManySep = function (r, n, i) {
					this.checkIsTarget(r, z.REPETITION_WITH_SEPARATOR, n, i) ||
						t.prototype.walkOption.call(this, r, n, i);
				}),
				e
			);
		...
		Oi = (function (t) {
			...
			return (
				(e.prototype.checkIsTarget = function (r, n) {
					r.idx === this.targetOccurrence &&
						this.targetProdType === n &&
						(this.targetRef === void 0 || r === this.targetRef) &&
						(this.result = r.definition);
				}),
				(e.prototype.visitOption = function (r) {
	...
	function Ni(t) {
		for (var e = new Array(t), r = 0; r < t; r++) e[r] = [];
		return e;
	}
	function wr(t) {
		for (var e = [""], r = 0; r < t.length; r++) {
	...
	function Dr(t, e) {
		for (
			var r = D.map(t, function (p) {
					return gi.possiblePathsFrom([p], 1);
				}),
				n = Ni(r.length),
				i = D.map(r, function (p) {
					var l = {};
					return (
						D.forEach(p, function (m) {
	...
	function Ri(t, e, r, n) {
		...
		var a = i.result,
			o = new os(e, t, r),
			s = o.startWalking(),
			c = new We.Alternative({ definition: a }),
			f = new We.Alternative({ definition: s });
		return Dr([c, f], n);
	}
...
var jr = R((x) => {
	...
	Object.defineProperty(x, "__esModule", { value: !0 });
	x.checkPrefixAlternativesAmbiguities =
		x.validateSomeNonEmptyLookaheadPath =
		x.validateTooManyAlts =
		x.RepetionCollector =
		x.validateAmbiguousAlternationAlternatives =
		x.validateEmptyOrAlternative =
		x.getFirstNoneTerminal =
		x.validateNoLeftRecursion =
		x.validateRuleIsOverridden =
		x.validateRuleDoesNotAlreadyExist =
		x.OccurrenceValidationCollector =
		x.identifyProductionForDuplicates =
		x.validateGrammar =
			void 0;
	var M = k(),
		B = k(),
	...
	function Mi(t) {
		return t instanceof _e.Terminal
			? t.terminalType.name
			: t instanceof _e.NonTerminal
			? t.nonTerminalName
			: "";
	}
	var bi = (function (t) {
	...
	function _t(t) {
		...
		var r = M.first(t);
		if (r instanceof _e.NonTerminal) e.push(r.referencedRule);
		else if (
			r instanceof _e.Alternative ||
			r instanceof _e.Option ||
			r instanceof _e.RepetitionMandatory ||
			r instanceof _e.RepetitionMandatoryWithSeparator ||
			r instanceof _e.RepetitionWithSeparator ||
			r instanceof _e.Repetition
		)
			e = e.concat(_t(r.definition));
		else if (r instanceof _e.Alternation)
			e = M.flatten(
				M.map(r.definition, function (o) {
	...
	function ds(t, e, r, n) {
		var i = [],
			a = B.reduce(
				...
				function (s, c, f) {
					return (
						e.definition[f].ignoreAmbiguities === !0 ||
							B.forEach(c, function (p) {
								var l = [f];
								B.forEach(t, function (m, v) {
									f !== v &&
										Qe.containsPath(m, p) &&
										e.definition[v].ignoreAmbiguities !== !0 &&
										l.push(v);
								}),
									l.length > 1 &&
										!Qe.containsPath(i, p) &&
										(i.push(p), s.push({ alts: l, path: p }));
							}),
						s
					);
				},
				...
				if (s.ignoreAmbiguities !== !0) {
					var c = o.idx,
						...
						p = B.findAll(a, function (m) {
							return (
								e.definition[m.idx].ignoreAmbiguities !== !0 &&
								m.idx < c &&
								Qe.isStrictPrefixOfPath(m.path, f)
							);
						}),
...
var et = R((ae) => {
	...
	ae.EarlyExitException =
		ae.NotAllInputParsedException =
		ae.NoViableAltException =
		ae.MismatchedTokenException =
		ae.isRecognitionException =
			void 0;
	var ys = k(),
...
var zr = R(($) => {
	...
	var Ps = (function () {
		function t() {}
		return (
			(t.prototype.initRecoverable = function (e) {
				(this.firstAfterRepMap = {}),
					(this.resyncFollows = {}),
					(this.recoveryEnabled = de.has(e, "recoveryEnabled")
						? e.recoveryEnabled
						: ks.DEFAULT_PARSER_CONFIG.recoveryEnabled),
					this.recoveryEnabled && (this.attemptInRepetitionRecovery = Vi);
			}),
			...
			(t.prototype.tryInRepetitionRecovery = function (e, r, n, i) {
				for (
					...
					if (this.tokenMatcher(l, i)) {
						...
						this.tokenMatcher(l, o)
							? (f = !0)
							: ((l = this.SKIP_TOKEN()), this.addToResyncTokens(l, c));
			...
			(t.prototype.canRecoverWithSingleTokenInsertion = function (e, r) {
				var n = this;
				if (!this.canTokenTypeBeInsertedInRecovery(e) || de.isEmpty(r))
					return !1;
				var i = this.LA(1),
					a =
						de.find(r, function (o) {
			...
			(t.prototype.attemptInRepetitionRecovery = function (
				e,
				r,
				n,
				i,
	...
	function Vi(t, e, r, n, i, a, o) {
		...
		this.RULE_STACK.length === 1 &&
			...
			m === void 0 &&
			((m = Ht.EOF), (v = 1)),
			this.shouldInRepetitionRecoveryBeTried(m, v, o) &&
				this.tryInRepetitionRecovery(t, e, r, m);
	}
...
var Yt = R((P) => {
	...
	P.getKeyForAutomaticLookahead =
		P.AT_LEAST_ONE_SEP_IDX =
		P.MANY_SEP_IDX =
		P.AT_LEAST_ONE_IDX =
		P.MANY_IDX =
		P.OPTION_IDX =
		...
		P.BITS_FOR_ALT_IDX =
		P.BITS_FOR_RULE_IDX =
		P.BITS_FOR_OCCURRENCE_IDX =
		P.BITS_FOR_METHOD_TYPE =
			void 0;
...
var zi = R((Xt) => {
	...
	var be = yt(),
		...
		xs = (function () {
			function t() {}
			return (
				(t.prototype.initLooksAhead = function (e) {
					(this.dynamicTokensEnabled = ge.has(e, "dynamicTokensEnabled")
						? e.dynamicTokensEnabled
						: Ki.DEFAULT_PARSER_CONFIG.dynamicTokensEnabled),
						(this.maxLookahead = ge.has(e, "maxLookahead")
							? e.maxLookahead
							: Ki.DEFAULT_PARSER_CONFIG.maxLookahead),
						(this.lookAheadFuncsCache = ge.isES2015MapSupported()
							? new Map()
							: []),
						ge.isES2015MapSupported()
						...
						r.TRACE_INIT(n.name + " Rule Lookahead", function () {
							...
							ge.forEach(a, function (l) {
								...
								r.TRACE_INIT("" + Ve.getProductionDslName(l) + m, function () {
									var v = be.buildLookaheadFuncForOr(
											l.idx,
											n,
											l.maxLookahead || r.maxLookahead,
											l.hasPredicates,
											r.dynamicTokensEnabled,
											r.lookAheadBuilderForAlternatives
										),
										u = Fe.getKeyForAutomaticLookahead(
											r.fullRuleNameToShort[n.name],
											Fe.OR_IDX,
											l.idx
										);
								...
								ge.forEach(o, function (l) {
									r.computeLookaheadFunc(
										...
										l.idx,
										Fe.MANY_IDX,
										be.PROD_TYPE.REPETITION,
										l.maxLookahead,
										Ve.getProductionDslName(l)
									);
								}),
								...
								ge.forEach(c, function (l) {
									r.computeLookaheadFunc(
										...
										l.idx,
										Fe.AT_LEAST_ONE_IDX,
										be.PROD_TYPE.REPETITION_MANDATORY,
										l.maxLookahead,
										Ve.getProductionDslName(l)
									);
								}),
								ge.forEach(f, function (l) {
									r.computeLookaheadFunc(
										n,
										l.idx,
										Fe.AT_LEAST_ONE_SEP_IDX,
										be.PROD_TYPE.REPETITION_MANDATORY_WITH_SEPARATOR,
										l.maxLookahead,
										Ve.getProductionDslName(l)
									);
								}),
								ge.forEach(p, function (l) {
									r.computeLookaheadFunc(
										...
										l.idx,
										Fe.MANY_SEP_IDX,
										be.PROD_TYPE.REPETITION_WITH_SEPARATOR,
										l.maxLookahead,
										Ve.getProductionDslName(l)
									);
								});
				...
				(t.prototype.computeLookaheadFunc = function (e, r, n, i, a, o) {
					...
					this.TRACE_INIT("" + o + (r === 0 ? "" : r), function () {
						var c = be.buildLookaheadFuncForOptionalProd(
								r,
								e,
								a || s.maxLookahead,
							...
							f = Fe.getKeyForAutomaticLookahead(
								s.fullRuleNameToShort[e.name],
								n,
								r
							);
				...
				(t.prototype.setLaFuncCacheUsingMap = function (e, r) {
...
var Hi = R((ke) => {
	...
	ke.addNoneTerminalToCst =
		ke.addTerminalToCst =
		ke.setNodeLocationFull =
		ke.setNodeLocationOnlyOffset =
			void 0;
	function Cs(t, e) {
...
var Hr = R((Be) => {
	...
	function Ds(t, e) {
		...
		return Fs.isUndefined(r) || r.configurable
			? (Object.defineProperty(t, Xi, {
					enumerable: !1,
					configurable: !0,
					writable: !1,
					value: e,
...
var ea = R((Y) => {
	...
	Y.validateRedundantMethods =
		Y.validateMissingCstMethods =
		Y.validateVisitor =
		Y.CstVisitorDefinitionError =
		Y.createBaseVisitorConstructorWithDefaults =
		Y.createBaseSemanticVisitorConstructor =
		Y.defaultVisit =
			void 0;
	var ve = k(),
		At = Hr();
				...
				if (!ve.isEmpty(i)) {
					...
					throw Error(
						"Errors Detected in CST Visitor <" +
							At.functionName(this.constructor) +
							`>:
	` +
							("" +
								a
...
`
	...
	function Qi(t, e) {
		var r = ve.map(e, function (n) {
			if (!ve.isFunction(t[n]))
				return {
					msg:
						"Missing visitor method: <" +
						n +
						"> on " +
						At.functionName(t.constructor) +
						" CST Visitor.",
...
var ra = R(($t) => {
	...
	var tt = Hi(),
		...
		qs = (function () {
			function t() {}
			return (
				(t.prototype.initTreeBuilder = function (e) {
					if (
						((this.CST_STACK = []),
						(this.outputCst = e.outputCst),
						(this.nodeLocationTracking = K.has(e, "nodeLocationTracking")
							? e.nodeLocationTracking
							: Bs.DEFAULT_PARSER_CONFIG.nodeLocationTracking),
						!this.outputCst)
					)
						(this.cstInvocationStateUpdate = K.NOOP),
							(this.cstFinallyStateUpdate = K.NOOP),
							(this.cstPostTerminal = K.NOOP),
							(this.cstPostNonTerminal = K.NOOP),
							(this.cstPostRule = K.NOOP);
					else if (/full/i.test(this.nodeLocationTracking))
						this.recoveryEnabled
							? ((this.setNodeLocationFromToken = tt.setNodeLocationFull),
							  (this.setNodeLocationFromNode = tt.setNodeLocationFull),
							  (this.cstPostRule = K.NOOP),
							  (this.setInitialNodeLocation =
									this.setInitialNodeLocationFullRecovery))
							: ((this.setNodeLocationFromToken = K.NOOP),
							  (this.setNodeLocationFromNode = K.NOOP),
							  (this.cstPostRule = this.cstPostRuleFull),
							  (this.setInitialNodeLocation =
									this.setInitialNodeLocationFullRegular));
					else if (/onlyOffset/i.test(this.nodeLocationTracking))
						this.recoveryEnabled
							? ((this.setNodeLocationFromToken = tt.setNodeLocationOnlyOffset),
							  (this.setNodeLocationFromNode = tt.setNodeLocationOnlyOffset),
							  (this.cstPostRule = K.NOOP),
							  (this.setInitialNodeLocation =
						...
						(this.setNodeLocationFromToken = K.NOOP),
							(this.setNodeLocationFromNode = K.NOOP),
							(this.cstPostRule = K.NOOP),
							(this.setInitialNodeLocation = K.NOOP);
					else
						throw Error(
				...
				(t.prototype.cstPostTerminal = function (e, r) {
					...
					tt.addTerminalToCst(n, r, e),
						this.setNodeLocationFromToken(n.location, r);
				}),
				(t.prototype.cstPostNonTerminal = function (e, r) {
					var n = this.CST_STACK[this.CST_STACK.length - 1];
					tt.addNoneTerminalToCst(n, r, e),
				...
				(t.prototype.getBaseCstVisitorConstructor = function () {
					if (K.isUndefined(this.baseCstVisitorConstructor)) {
						var e = ta.createBaseSemanticVisitorConstructor(
							this.className,
							K.keys(this.gastProductionsCache)
						);
				...
				(t.prototype.getBaseCstVisitorConstructorWithDefaults = function () {
					if (K.isUndefined(this.baseCstVisitorWithDefaultsConstructor)) {
						var e = ta.createBaseVisitorConstructorWithDefaults(
							this.className,
							K.keys(this.gastProductionsCache),
		...
		js = (function () {
			...
			return (
				...
				(t.prototype.LA = function (e) {
					...
					return r < 0 || this.tokVectorLength <= r
						? na.END_OF_FILE
						: this.tokVector[r];
				}),
			...
			return (
				...
				(t.prototype.RULE = function (e, r, n) {
					if (
						...
						aa.contains(this.definedRulesNames, e))
					) {
						var i =
								Ks.defaultGrammarValidatorErrorProvider.buildDuplicateRuleNameError(
									{ topLevelRule: e, grammarName: this.className }
								),
							a = {
								message: i,
...
var la = R((Jt) => {
	...
	var q = k(),
		...
		Zs = (function () {
			function t() {}
			return (
				(t.prototype.initRecognizerEngine = function (e, r) {
					if (
						((this.className = $s.classNameFromInstance(this)),
						(this.shortRuleNameToFull = {}),
						(this.fullRuleNameToShort = {}),
						(this.ruleShortNameIdx = 256),
						(this.tokenMatcher = Rt.tokenStructuredMatcherNoCategories),
						(this.definedRulesNames = []),
						(this.tokensMap = {}),
	...
	For Further details.`);
					...
					if (q.isArray(e))
						...
						q.has(e, "modes") &&
						q.every(q.flatten(q.values(e.modes)), Rt.isTokenType)
					) {
					...
					} else if (q.isObject(e)) this.tokensMap = q.cloneObj(e);
					else
						throw new Error(
					...
					(this.tokenMatcher = a
						? Rt.tokenStructuredMatcherNoCategories
						: Rt.tokenStructuredMatcher),
						Rt.augmentTokenTypes(q.values(this.tokensMap));
				}),
...
Make sure that all grammar rule definitions are done before 'performSelfAnalysis' is called.`
					...
					var i = q.has(n, "resyncEnabled")
							? n.resyncEnabled
							: ua.DEFAULT_RULE_CONFIG.resyncEnabled,
						a = q.has(n, "recoveryValueFunc")
							? n.recoveryValueFunc
							: ua.DEFAULT_RULE_CONFIG.recoveryValueFunc,
						o =
							this.ruleShortNameIdx <<
							(le.BITS_FOR_METHOD_TYPE + le.BITS_FOR_OCCURRENCE_IDX);
					this.ruleShortNameIdx++,
					...
					var c = function (p, l) {
							return (
								p === void 0 && (p = 0),
								this.ruleInvocationStateUpdate(o, e, p),
								s.call(this, l)
							);
						},
						f = "ruleName";
				...
				(t.prototype.atLeastOneInternalLogic = function (e, r, n) {
					...
					if (a.call(this) === !0)
						for (
							var f = this.doSingleRepetition(o);
							a.call(this) === !0 && f === !0;
						...
						throw this.raiseEarlyExitException(
							e,
							sa.PROD_TYPE.REPETITION_MANDATORY,
							r.ERR_MSG
						);
					this.attemptInRepetitionRecovery(
						...
						le.AT_LEAST_ONE_IDX,
						e,
						rt.NextTerminalAfterAtLeastOneWalker
					);
				}),
				...
				(t.prototype.atLeastOneSepFirstInternalLogic = function (e, r, n) {
					...
					if (s.call(this) === !0) {
						...
						this.attemptInRepetitionRecovery(
							...
							[e, o, c, a, rt.NextTerminalAfterAtLeastOneSepWalker],
							c,
							le.AT_LEAST_ONE_SEP_IDX,
							e,
							rt.NextTerminalAfterAtLeastOneSepWalker
						);
					} else
						throw this.raiseEarlyExitException(
							e,
							sa.PROD_TYPE.REPETITION_MANDATORY_WITH_SEPARATOR,
							r.ERR_MSG
						);
				}),
				...
				(t.prototype.manyInternalLogic = function (e, r, n) {
					...
					this.attemptInRepetitionRecovery(
						...
						le.MANY_IDX,
						e,
						rt.NextTerminalAfterManyWalker,
						f
					);
				}),
				...
				(t.prototype.manySepFirstInternalLogic = function (e, r, n) {
					...
					if (s.call(this) === !0) {
						...
						this.attemptInRepetitionRecovery(
							this.repetitionSepSecondInternal,
							[e, o, c, a, rt.NextTerminalAfterManySepWalker],
							c,
							le.MANY_SEP_IDX,
							e,
				...
				(t.prototype.consumeInternal = function (e, r, n) {
					...
					try {
						...
						this.tokenMatcher(a, e) === !0
							...
							: this.consumeInternalError(e, a, n);
					} catch (o) {
				...
				(t.prototype.ruleInvocationStateUpdate = function (e, r, n) {
					this.RULE_OCCURRENCE_STACK.push(n),
						...
						this.cstInvocationStateUpdate(r, e);
				}),
...
var pa = R((tr) => {
	...
	var $r = et(),
		...
		Js = (function () {
			function t() {}
			return (
				(t.prototype.initErrorHandler = function (e) {
					(this._errors = []),
						(this.errorMessageProvider = Zr.has(e, "errorMessageProvider")
				...
				(t.prototype.raiseEarlyExitException = function (e, r, n) {
					for (
						var i = this.getCurrRuleFullName(),
							...
							o = fa.getLookaheadPathsForOptionalProd(
								e,
								a,
								r,
								this.maxLookahead
					...
					throw this.SAVE_ERROR(
						new $r.EarlyExitException(p, this.LA(1), this.LA(0))
					);
				}),
				(t.prototype.raiseNoAltException = function (e, r) {
					for (
						var n = this.getCurrRuleFullName(),
							...
							a = fa.getLookaheadPathsForOr(e, i, this.maxLookahead),
							o = [],
							s = 1;
						s <= this.maxLookahead;
						s++
		...
		eu = (function () {
			...
			return (
				...
				(t.prototype.computeContentAssist = function (e, r) {
					...
					return ha.nextPossibleTokensAfter(
						[n],
						r,
						this.tokenMatcher,
						this.maxLookahead
...
var Ra = R((nr) => {
	...
	See: https://chevrotain.io/docs/guide/internals.html#grammar-recording for details`,
		...
		ou = (function () {
			function t() {}
			return (
				(t.prototype.initGastRecorder = function (e) {
				...
				(t.prototype.topLevelRuleRecord = function (e, r) {
					try {
						var n = new Pe.Rule({ definition: [], name: e });
						return (
							(n.name = e),
 ...
 inside top level rule: <` +
					...
					var a = oe.peek(this.recordingProdStack),
						o = new Pe.Terminal({ idx: r, terminalType: e });
	...
	function Ot(t, e, r, n) {
		...
		return (
			n && (o.separator = e.SEP),
			oe.has(e, "MAX_LOOKAHEAD") && (o.maxLookahead = e.MAX_LOOKAHEAD),
	...
	function au(t, e) {
		...
		return (
			(o.hasPredicates = s),
			n.definition.push(o),
			oe.forEach(a, function (c) {
				var f = new Pe.Alternative({ definition: [] });
				o.definition.push(f),
					oe.has(c, "IGNORE_AMBIGUITIES")
						? (f.ignoreAmbiguities = c.IGNORE_AMBIGUITIES)
						: oe.has(c, "GATE") && (f.ignoreAmbiguities = !0),
					r.recordingProdStack.push(f),
					c.ALT.call(r),
...
var Na = R((or) => {
	...
	var Oa = k(),
		...
		uu = (function () {
			function t() {}
			return (
				(t.prototype.initPerformanceTracer = function (e) {
					if (Oa.has(e, "traceInitPerf")) {
						var r = e.traceInitPerf,
							n = typeof r == "number";
						(this.traceInitMaxIdent = n ? r : Infinity),
							(this.traceInitPerf = n ? r > 0 : r);
					} else
						(this.traceInitMaxIdent = 0),
							(this.traceInitPerf = su.DEFAULT_PARSER_CONFIG.traceInitPerf);
					this.traceInitIndent = -1;
				}),
				(t.prototype.TRACE_INIT = function (e, r) {
					if (this.traceInitPerf === !0) {
...
var ce = R((U) => {
	...
	U.EmbeddedActionsParser =
		U.CstParser =
		U.Parser =
		U.EMPTY_ALT =
		U.ParserDefinitionErrorType =
		U.DEFAULT_RULE_CONFIG =
	...
	var ee = k(),
		...
		_u = Na(),
		gu = Ia();
	U.END_OF_FILE = Pa.createTokenInstance(
		Pa.EOF,
		"",
		NaN,
		NaN,
	...
	var ur = (function () {
		function t(e, r) {
			...
			if (
				...
				ee.has(r, "ignoredIssues"))
			)
				throw new Error(`The <ignoredIssues> IParserConfig property has been deprecated.
	Please use the <IGNORE_AMBIGUITIES> flag on the relevant DSL method instead.
	See: https://chevrotain.io/docs/guide/resolving_grammar_errors.html#IGNORING_AMBIGUITIES
	For further details.`);
			this.skipValidations = ee.has(r, "skipValidations")
				? r.skipValidations
				: U.DEFAULT_PARSER_CONFIG.skipValidations;
		}
		return (
			(t.performSelfAnalysis = function (e) {
				throw Error(
				...
				this.TRACE_INIT("performSelfAnalysis", function () {
					...
					e.TRACE_INIT("toFastProps", function () {
						...
						e.TRACE_INIT("Grammar Recording", function () {
							try {
								e.enableRecording(),
									ee.forEach(e.definedRulesNames, function (a) {
										var o = e[a],
					...
					if (
						...
						ee.isEmpty(e.definitionErrors) &&
							(e.recoveryEnabled &&
								e.TRACE_INIT("computeAllProdsFollows", function () {
									var a = lu.computeAllProdsFollows(
										ee.values(e.gastProductionsCache)
									);
									e.resyncFollows = a;
								}),
							e.TRACE_INIT("ComputeLookaheadFunctions", function () {
								e.preComputeLookaheadFunctions(
									ee.values(e.gastProductionsCache)
								);
							})),
						!t.DEFER_DEFINITION_ERRORS_HANDLING &&
							!ee.isEmpty(e.definitionErrors))
					)
						throw (
							((r = ee.map(e.definitionErrors, function (a) {
								return a.message;
							})),
							new Error(
...
var La = R((cr) => {
	...
	function Iu(t, e) {
		var r = e === void 0 ? {} : e,
			...
			i =
				n === void 0
					? "https://unpkg.com/chevrotain@" + Ca.VERSION + "/diagrams/"
					: n,
			a = r.css,
			o =
				a === void 0
