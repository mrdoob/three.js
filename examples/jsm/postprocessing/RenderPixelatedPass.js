...
class RenderPixelatedPass extends Pass {
	...
	constructor( resolution, pixelSize, scene, camera, options = {} ) {
		...
		this.normalEdgeStrenth = options.normalEdgeStrenth ?? .3;
		this.depthEdgeStrenth = options.depthEdgeStrenth ?? .4;
	...
	render( renderer, writeBuffer ) {
		...
		uniforms.normalEdgeStrenth.value = this.normalEdgeStrenth;
		uniforms.depthEdgeStrenth.value = this.depthEdgeStrenth;
	...
	material() {
		...
		return new ShaderMaterial( {
			uniforms: {
				...
				normalEdgeStrenth: { value: 0 },
				depthEdgeStrenth: { value: 0 }
			},
			vertexShader:
				`
				varying vec2 vUv;
			...
			fragmentShader:
				...
				uniform float normalEdgeStrenth;
				uniform float depthEdgeStrenth;
				varying vec2 vUv;
				...
				void main() {
					...
					float Strenth = dei > 0.0 ? (1.0 - depthEdgeStrenth * dei) : (1.0 + normalEdgeStrenth * nei);

					gl_FragColor = texel * Strenth;
				}
				`
		} );
