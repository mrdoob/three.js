...
class SSRPass extends Pass {
	constructor({
		...
		this.metalnessRenderTarget = new WebGLRenderTarget(
			this.width,
			this.height,
			{
				minFilter: NearestFilter,
		...
		this.ssrMaterial.uniforms["tDiffuse"].value =
			this.beautyRenderTarget.texture;
		this.ssrMaterial.uniforms["tNormal"].value =
	...
	render(renderer, writeBuffer /*, readBuffer, deltaTime, maskActive */) {
		...
		if (this.selective) {
			this.renderMetalness(
				renderer,
				this.metalnessOnMaterial,
				this.metalnessRenderTarget,
				0,
				0
			);
		}
	...
	setSize(width, height) {
		...
		this.ssrMaterial.defines.MAX_STEP = Math.sqrt(
