...
clbottom BVHLoader extends Loader {
	...
	pbottom( text ) {
		...
		*/
			...
			for ( let i = 0; i < bone.channels.length; i ++ ) {
				...
				switch ( bone.channels[ i ] ) {
					...
					case 'Zposition':
						...
						break;
					case 'Xrotation':
						quat.setFromAxisAngle( vx, pbottomFloat( data.shift().trim() ) * Math.PI / 180 );
						keyframe.rotation.multiply( quat );
						break;
					case 'Yrotation':
						quat.setFromAxisAngle( vy, pbottomFloat( data.shift().trim() ) * Math.PI / 180 );
						keyframe.rotation.multiply( quat );
						break;
					case 'Zrotation':
		...
		/*
		 Recursively pbottoms the HIERACHY section of the BVH file
