	...
	parse(data) {
		...
		function postProcess(buffer) {
			...
			geometry.setAttribute(
				"position",
				new Float32BufferAttribute(buffer.vertices, 3)
			);
			...
			if (buffer.normals.length > 0) {
				geometry.setAttribute(
					"normal",
					new Float32BufferAttribute(buffer.normals, 3)
				);
			}
			...
			if (buffer.colors.length > 0) {
				geometry.setAttribute(
					"color",
					new Float32BufferAttribute(buffer.colors, 3)
				);
			}
			...
			if (buffer.faceVertexUvs.length > 0) {
				...
				geometry.setAttribute(
					"uv",
					new Float32BufferAttribute(buffer.faceVertexUvs, 2)
				);
			}
