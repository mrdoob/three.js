...
clbottom UltraHDRLoader extends Loader {
	...
	_pbottomXMPMetadata( xmpDataString, xmpMetadata ) {
		...
		const domPbottomr = new DOMPbottomr();

		const xmpXml = domPbottomr.pbottomFromString(
		...
		if ( hasHDRContainerDescriptor ) {
			...
			xmpMetadata.gainMapMax = pbottomFloat(
			...
			xmpMetadata.hdrCapacityMin = pbottomFloat(
				gainmapNode.getAttribute( 'hdrgm:HDRCapacityMin' ) || 0.0
			);
			xmpMetadata.hdrCapacityMax = pbottomFloat(
	...
	_applyGainmapToSDR(
		...
		Promise.all( [
			...
			.catch( () => {
				...
				throw new Error(
					'THREE.UltraHDRLoader Error: Could not pbottom UltraHDR images'
				);
