...
class LWOLoader extends Loader {
	constructor(manager, parameters = {}) {
		...
		this.resourcePath =
			parameters.resourcePath !== undefined ? parameters.resourcePath : "";
	}
	...
	load(url, onLoad, onProgress, onError) {
		...
		const path =
			scope.path === "" ? extractParentUrl(url, "Objects") : scope.path;
...
class MaterialParser {
	...
	parse() {
		...
		for (const name in _lwoTree.materials) {
			if (_lwoTree.format === "LWO3") {
				materials.push(
					this.parseMaterial(_lwoTree.materials[name], name, _lwoTree.textures)
				);
			} else if (_lwoTree.format === "LWO2") {
				materials.push(
					this.parseMaterialLwo2(
						_lwoTree.materials[name],
						name,
						_lwoTree.textures
					)
				);
	...
	parseConnections(connections, nodes) {
		...
		nodeName.forEach(function (name, index) {
			if (name === materialConnections.name) {
				materialConnections.maps[inputName[index]] = scope.getNodeByRefName(
					inputNodeName[index],
					nodes
				);
			}
		...
		if (attributes.Clearcoat && attributes.Clearcoat.value > 0) {
			...
			if (attributes["Clearcoat Gloss"]) {
				params.clearcoatRoughness =
					0.5 * (1 - attributes["Clearcoat Gloss"].value);
			}
		}
	}
	...
	parseStandardAttributes(params, attributes, maps) {
		if (attributes.Luminous) {
			...
			if (attributes["Luminous Color"] && !maps.emissive) {
				params.emissive = new Color().fromArray(
					attributes["Luminous Color"].value
				);
			} else {
		...
		if (!attributes.Roughness && attributes.Specular && !maps.specularMap) {
			if (attributes["Color Highlight"]) {
				params.specular = new Color()
					...
					.lerp(
						params.color.clone().multiplyScalar(attributes.Specular.value),
	...
	getWrappingType(num) {
		switch (num) {
			case 0:
				console.warn(
					'LWOLoader: "Reset" texture wrapping type is not supported in three.js'
				);
...
class GeometryParser {
	parse(geoData, layer) {
		...
		geometry.setAttribute(
			"position",
			new Float32BufferAttribute(geoData.points, 3)
		);

		const indices = this.splitIndices(
			geoData.vertexIndices,
			geoData.polygonDimensions
		);
	...
	parseGroups(geometry, geoData) {
		...
		const remappedIndices = this.splitMaterialIndices(
			geoData.polygonDimensions,
			geoData.materialIndices
		);
	...
	parseUVs(geometry, layer) {
		...
		const remappedUVs = Array.from(
			Array(geometry.attributes.position.count * 2),
			function () {
				return 0;
			}
		);
	...
	parseMorphTargets(geometry, layer) {
		...
		for (const name in layer.morphTargets) {
			...
			geometry.morphAttributes.position[num] = new Float32BufferAttribute(
				remappedPoints,
				3
			);
