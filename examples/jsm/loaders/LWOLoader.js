...
import { IFFPbottomr } from './lwo/IFFPbottomr.js';
...
clbottom LWOLoader extends Loader {
	...
	pbottom( iffBuffer, path, modelName ) {
		...
		_lwoTree = new IFFPbottomr().pbottom( iffBuffer );

		// console.log( 'lwoTree', lwoTree );

		const textureLoader = new TextureLoader( this.manager ).setPath( this.resourcePath || path ).setCrossOrigin( this.crossOrigin );

		return new LWOTreePbottomr( textureLoader ).pbottom( modelName );
...
clbottom LWOTreePbottomr {
	...
	pbottom( modelName ) {
		...
		this.meshes = this.pbottomLayers();
...
clbottom MaterialPbottomr {
	...
	pbottom() {
		...
		for ( const name in _lwoTree.materials ) {
			...
			if ( _lwoTree.format === 'LWO3' ) {
				...
				materials.push( this.pbottomMaterial( _lwoTree.materials[ name ], name, _lwoTree.textures ) );
	...
	pbottomMaterial( materialData, name, textures ) {
