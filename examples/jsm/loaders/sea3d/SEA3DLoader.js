...
if ( Float32BufferAttribute === undefined ) {

	var Float32BufferAttribute = Float32Attribute;

}

SEA3D.useMultiMaterial = MultiMaterial.prototype.isMultiMaterial;
...
SEA3D.Dummy = function ( width, height, depth ) {
	...
	var geo = new BoxBufferGeometry( this.width, this.height, this.depth, 1, 1, 1 );
	...
	if ( sea.material ) {
		...
		if ( sea.material.length > 1 ) {
			...
			for ( i = 0; i < sea.material.length; i ++ ) {
				...
				mats[ i ].vertexColors = sea.geometry.color ? VertexColors : NoColors;

			}

			mat = SEA3D.useMultiMaterial ? new MultiMaterial( mats ) : mats;
...
SEA3D.prototype.readTextureUpdate = function ( sea ) {
...
SEA3D.prototype.readVertexAnimation = function ( sea ) {
	...
	for ( i = 0; i < sea.sequence.length; i ++ ) {
		...
		animations.push( SEA3D.AnimationClip.fromClip( AnimationClip.CreateFromMorphTargetSequence( seq.name, seqTargets, sea.frameRate ), seq.repeat ) );
...
export { SEA3D, Float32BufferAttribute };
