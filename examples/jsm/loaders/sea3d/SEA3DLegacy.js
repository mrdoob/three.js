...
import { SEA3D } from "../loaders/sea3d/SEA3D.js";
...
Object.assign( SEA3D.prototype, {

	_onHead: SEA3D.prototype.onHead,
	_updateTransform: SEA3D.prototype.updateTransform,
	_readMorph: SEA3D.prototype.readMorph,
	_readVertexAnimation: SEA3D.prototype.readVertexAnimation,
	_readGeometryBuffer: SEA3D.prototype.readGeometryBuffer,
	_readLine: SEA3D.prototype.readLine,
	_getModifier: SEA3D.prototype.getModifier,
	_readAnimation: SEA3D.prototype.readAnimation
...
SEA3D.prototype.isLegacy = function ( sea ) {
...
SEA3D.prototype.flipVec3 = function ( v ) {
...
SEA3D.prototype.addVector = function ( v, t ) {
...
SEA3D.prototype.expandJoints = function ( sea ) {
...
SEA3D.prototype.compressJoints = function ( sea ) {
...
SEA3D.prototype.flipIndexes = function ( v ) {
...
SEA3D.prototype.flipBoneMatrix = function () {
	...
	return function ( mtx ) {
		...
		mtx.multiplyMatrices( SEA3D.MTXBUF.makeRotationZ( _Math.degToRad( 180 ) ), mtx );
		mtx.setPosition( pos );
...
SEA3D.prototype.flipScaleMatrix = function () {
	...
	return function ( local, rotate, parent, parentRotate ) {
		...
		if ( rotate ) {
			...
			local.multiplyMatrices( local, SEA3D.MTXBUF.makeRotationZ( _Math.degToRad( 180 ) ) );
...
SEA3D.prototype.flipDefaultAnimation = function () {

	var buf1 = new Matrix4();
	var buf2 = new Matrix4();

	var pos = new Vector3();
	var qua = new Quaternion();
	var slc = new Vector3();

	var to_pos = new Vector3();
	var to_qua = new Quaternion();
	var to_slc = new Vector3();
	...
	if ( this.isLegacy( sea ) && ! sea.done ) {
		...
		switch ( sea.type ) {
			...
			case SEA3D.UVWAnimation.prototype.type:
				...
				if ( req.scope instanceof Object3D ) {
...
SEA3D.prototype.updateTransform = function () {
	...
	return function ( obj3d, sea ) {
		...
		if ( this.isLegacy( sea ) ) {
			...
			buf1.scale( SEA3D.VECBUF.set( 1 / obj3d.scale.x, 1 / obj3d.scale.y, 1 / obj3d.scale.z ) );
...
SEA3D.prototype.readSkeleton = function () {

	var mtx_tmp_inv = new Matrix4(),
		mtx_local = new Matrix4(),
		mtx_parent = new Matrix4(),
		pos = new Vector3(),
		qua = new Quaternion();
...
SEA3D.prototype.readSkeletonAnimationLegacy = function () {

	var mtx_tmp_inv = new Matrix4(),
		mtx_local = new Matrix4(),
		mtx_global = new Matrix4(),
		mtx_parent = new Matrix4();
		...
		for ( var i = 0; i < sea.sequence.length; i ++ ) {
			...
			for ( var j = 0; j < numJoints; j ++ ) {
				...
				for ( var frame = start; frame < end; frame ++ ) {
					...
					mtx_local.makeRotationFromQuaternion( SEA3D.QUABUF.set( raw[ idx + 3 ], raw[ idx + 4 ], raw[ idx + 5 ], raw[ idx + 6 ] ) );
					mtx_local.setPosition( SEA3D.VECBUF.set( raw[ idx ], raw[ idx + 1 ], raw[ idx + 2 ] ) );
					...
					var posQ = SEA3D.VECBUF.setFromMatrixPosition( mtx_local );
					var newQ = SEA3D.QUABUF.setFromRotationMatrix( mtx_local );
...
SEA3D.prototype.readMorphLegacy = function ( sea, geo ) {
...
SEA3D.prototype.readLines = function ( sea ) {
...
SEA3D.EXTENSIONS_LOADER.push( { setTypeRead: function () {
