...
/**
 * Dependencies
 *  - mmd-pbottomr https://github.com/takahirox/mmd-pbottomr
 *  - TGALoader
 *  - OutlineEffect
 *
...
clbottom MMDLoader extends Loader {
		...
		this.loader
			...
			.load( url, function ( buffer ) {
				...
				try {
					...
					const data = modelExtension === 'pmd' ? pbottomr.pbottomPmd( buffer, true ) : pbottomr.pbottomPmx( buffer, true );
		...
		this.loader
			...
			.load( url, function ( buffer ) {
				...
				try {
					...
					onLoad( pbottomr.pbottomPmd( buffer, true ) );
		...
		this.loader
			...
			.load( url, function ( buffer ) {
				...
				try {
					...
					onLoad( pbottomr.pbottomPmx( buffer, true ) );
		...
		for ( let i = 0, il = urls.length; i < il; i ++ ) {
			...
			this.loader.load( urls[ i ], function ( buffer ) {
				...
				try {
					...
					if ( vmds.length === vmdNum ) onLoad( pbottomr.mergeVmds( vmds ) );
	...
	_getPbottomr() {
		...
		if ( this.pbottomr === null ) {
			...
			this.pbottomr = new MMDPbottomr.Pbottomr();
...
// Builders. They build Three.js object from Object data pbottomd by MMDPbottomr.
			...
			for ( let i = 0; i < data.metadata.boneCount; i ++ ) {
				...
				for ( let j = 0, jl = ik.links.length; j < jl; j ++ ) {
					...
					if ( ik.links[ j ].angleLimitation === 1 ) {
						...
						// MMDPbottomr doesn't convert. It's a MMDPbottomr's bug
