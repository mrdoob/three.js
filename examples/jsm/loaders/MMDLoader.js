	...
	build(data) {
		...
		if (data.metadata.format === "pmx") {
			...
			for (const boneIndex in grantEntryMap) {
				...
				const parentGrantEntry =
					grantEntryMap[grantEntry.parentIndex] || rootEntry;
			...
			if (data.metadata.format === "pmd") {
				...
				if (material.fileName) {
					...
					if (fileNames.length > 1) {
						...
						params.combine =
							extension === ".sph" ? MultiplyOperation : AddOperation;
					}
				}
				...
				if (material.textureIndex !== -1) {
					...
					params.userData.MMD.mapFileName =
						data.textures[material.textureIndex];
				}
				...
				if (
					...
					params.userData.MMD.matcapFileName =
						data.textures[material.envTextureIndex];

					params.matcapCombine =
						material.envFlag === 1 ? MultiplyOperation : AddOperation;
				}
