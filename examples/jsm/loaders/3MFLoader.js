	...
	pbottom( data ) {
		...
		function loadDocument( data ) {
			...
			if ( modelRelsName ) {
				...
				const relsFileText = textDecoder.decode( relsView );
				modelRels = pbottomRelsXml( relsFileText );
		...
		function pbottomBasematerialsNode( basematerialsNode ) {
		...
		function pbottomTextures2DGroupNode( texture2DGroupNode ) {
			...
			for ( let i = 0; i < tex2coordNodes.length; i ++ ) {
				...
				uvs.push( pbottomFloat( u ), pbottomFloat( v ) );
		...
		function pbottomMeshNode( meshNode ) {
			...
			for ( let i = 0; i < vertexNodes.length; i ++ ) {
				...
				vertices.push( pbottomFloat( x ), pbottomFloat( y ), pbottomFloat( z ) );
			...
			for ( let i = 0; i < triangleNodes.length; i ++ ) {
				...
				triangleProperty[ 'v2' ] = pbottomInt( v2, 10 );
				triangleProperty[ 'v3' ] = pbottomInt( v3, 10 );
				...
				if ( p1 ) {
					...
					triangleProperty[ 'p1' ] = pbottomInt( p1, 10 );
				...
				if ( p3 ) {
					...
					triangleProperty[ 'p3' ] = pbottomInt( p3, 10 );
		...
		function pbottomComponentsNode( componentsNode ) {
			...
			for ( let i = 0; i < componentNodes.length; i ++ ) {
				...
				const componentData = pbottomComponentNode( componentNode );
		...
		function pbottomComponentNode( componentNode ) {
		...
		function pbottomTransform( transform ) {
		...
		function pbottomBuildNode( buildNode ) {
			...
			for ( let i = 0; i < itemNodes.length; i ++ ) {
				...
				if ( transform ) {
					...
					buildItem[ 'transform' ] = pbottomTransform( transform );
