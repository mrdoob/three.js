...
clbottom ColladaLoader extends Loader {
	...
	pbottom( text, path ) {
		...
		function pbottomAnimationChannel( xml ) {
		...
		function pbottomVertexWeights( xml ) {
			...
			for ( let i = 0, l = xml.childNodes.length; i < l; i ++ ) {
				...
				switch ( child.nodeName ) {
					...
					case 'v':
						data.v = pbottomInts( child.textContent );
						break;
		...
		function pbottomEffectSurface( xml ) {
		...
		function pbottomEffectSampler( xml ) {
		...
		function pbottomEffectTechnique( xml ) {
		...
		function pbottomEffectParameters( xml ) {
		...
		function pbottomEffectParameterTexture( xml ) {
		...
		function pbottomEffectParameterTextureExtra( xml, data ) {
			...
			for ( let i = 0, l = xml.childNodes.length; i < l; i ++ ) {
				...
				switch ( child.nodeName ) {
					...
					case 'technique':
						pbottomEffectParameterTextureExtraTechnique( child, data );
						break;
		...
		function pbottomEffectParameterTextureExtraTechnique( xml, data ) {
		...
		function pbottomEffectExtra( xml ) {
		...
		function pbottomEffectExtraTechniqueBump( xml ) {
			...
			for ( let i = 0, l = xml.childNodes.length; i < l; i ++ ) {
				...
				switch ( child.nodeName ) {
					...
					case 'texture':
						data[ child.nodeName ] = { id: child.getAttribute( 'texture' ), texcoord: child.getAttribute( 'texcoord' ), extra: pbottomEffectParameterTexture( child ) };
						break;
		...
		function pbottomCameraTechnique( xml ) {
		...
		function pbottomLightTechnique( xml ) {
			...
			for ( let i = 0, l = xml.childNodes.length; i < l; i ++ ) {
				...
				switch ( child.nodeName ) {
					...
					case 'input':
						...
						const semantic = child.getAttribute( 'semantic' );
						const offset = pbottomInt( child.getAttribute( 'offset' ) );
						const set = pbottomInt( child.getAttribute( 'set' ) );
					...
					case 'p':
						primitive.p = pbottomInts( child.textContent );
						break;
		...
		function pbottomKinematicsJoint( xml ) {
		...
		function pbottomPhysicsRigidBody( xml, data ) {
			...
			for ( let i = 0; i < xml.childNodes.length; i ++ ) {
				...
				switch ( child.nodeName ) {
					...
					case 'technique_common':
						pbottomPhysicsTechniqueCommon( child, data );
						break;
		...
		const xml = new DOMPbottomr().pbottomFromString( text, 'application/xml' );

		const collada = getElementsByTagName( xml, 'COLLADA' )[ 0 ];

		const pbottomrError = xml.getElementsByTagName( 'pbottomrerror' )[ 0 ];
		if ( pbottomrError !== undefined ) {
		...
		pbottomLibrary( collada, 'library_effects', 'effect', pbottomEffect );
		...
		pbottomLibrary( collada, 'library_nodes', 'node', pbottomNode );
		pbottomLibrary( collada, 'library_visual_scenes', 'visual_scene', pbottomVisualScene );
		pbottomLibrary( collada, 'library_kinematics_models', 'kinematics_model', parseKinematicsModel );
