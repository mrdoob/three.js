	...
	pbottom( text ) {
		...
		function pbottomNode( node, style ) {
			...
			switch ( node.nodeName ) {
				...
				case 'ellipse':
					style = pbottomStyle( node, style );
					path = pbottomEllipseNode( node );
					break;

				case 'line':
					style = pbottomStyle( node, style );
					path = pbottomLineNode( node );
					break;
		...
		function pbottomPathNode( node ) {
			...
			for ( let i = 0, l = commands.length; i < l; i ++ ) {
				...
				switch ( type ) {
					...
					case 'A':
						numbers = pbottomFloats( data, [ 3, 4 ], 7 );
		...
		*/
		...
		function pbottomCircleNode( node ) {
			...
			const r = pbottomFloatWithUnits( node.getAttribute( 'r' ) || 0 );
		...
		function pbottomNodeTransform( node ) {
			...
			if ( node.nodeName === 'use' && ( node.hasAttribute( 'x' ) || node.hasAttribute( 'y' ) ) ) {
				...
				const ty = pbottomFloatWithUnits( node.getAttribute( 'y' ) );
