...
import {
	...
	WebGPUNodeSampledCubeTexture,
} from "./WebGPUNodeSampledTexture.js";
...
class WebGPUNodeBuilder extends NodeBuilder {
	...
	getBindings() {
		...
		return this.material !== null
			? [...bindings.vertex, ...bindings.fragment]
			: bindings.compute;
	}
	...
	getUniformFromNode(node, shaderStage, type) {
		...
		if (nodeData.uniformGPU === undefined) {
			...
			if (type === "texture" || type === "cubeTexture") {
				const sampler = new WebGPUNodeSampler(
				...
				if (type === "texture") {
					texture = new WebGPUNodeSampledTexture(
						uniformNode.name,
						uniformNode.node
					);
				} else if (type === "cubeTexture") {
					texture = new WebGPUNodeSampledCubeTexture(
						uniformNode.name,
						uniformNode.node
					);
				}
				...
				const index =
					lastBinding && lastBinding.isUniformsGroup
						? bindings.length - 1
						: bindings.length;
				...
				const bufferClass =
					type === "storageBuffer" ? WebGPUStorageBuffer : WebGPUUniformBuffer;
	...
	getVaryings(shaderStage) {
		...
		if (shaderStage === "vertex") {
			...
			for (let index = 0; index < varyings.length; index++) {
				...
				if (varying.needsInterpolation) {
					snippets.push(
						`@location( ${index} ) ${varying.name} : ${this.getType(
							varying.type
						)}`
					);
		...
		for (const { name, property, type } of this.builtins[
			shaderStage
		].values()) {
			snippets.push(`@builtin( ${name} ) ${property} : ${type}`);
		}
		...
		return shaderStage === "vertex"
			? this._getWGSLStruct("NodeVaryingsStruct", "\t" + code)
			: code;
	}
	...
	getUniforms(shaderStage) {
		...
		if (groupSnippets.length > 0) {
			code += this._getWGSLStructBinding(
				"NodeUniforms",
				groupSnippets.join(",\n"),
				"uniform",
				index++
