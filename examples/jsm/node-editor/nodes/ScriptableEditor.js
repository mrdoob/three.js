...
import { disposeScene, getColorFromValue, getColorFromType, createElementFromJSON, isGPUNode, onValidType, getTypeFromValue } from '../NodeEditorUtils.js';
...
export class ScriptableEditor extends BaseNodeEditor {
	...
	updateOutputInEditor() {
		...
		if ( editor ) {
			...
			} else if ( editorOutputAdded && editorOutputAdded.isPass === true ) {

				global.get( 'composer' ).removePass( editorOutputAdded );

			}

			if ( editorOutput && editorOutput.isObject3D === true ) {

				global.get( 'scene' ).add( editorOutput );

			} else if ( editorOutput && editorOutput.isPass === true ) {

				global.get( 'composer' ).addPass( editorOutput );
