...
const RE_IndirectDiffuse_Physical = new ShaderNode((inputs) => {
	...
	reflectedLight.indirectDiffuse.add(
		mul(irradiance, BRDF_Lambert.call({ diffuseColor }))
	);
});
...
const RE_Direct_Physical = new ShaderNode((inputs) => {
	...
	reflectedLight.directDiffuse.add(
		mul(irradiance, BRDF_Lambert.call({ diffuseColor: diffuseColor.rgb }))
	);
