...
export const func = (code, includes) => {
	...
	node.call = (...params) =>
		nodeObject(
			call(
				params.length > 1 || params[0]?.isNode === true
					? nodeArray(params)
...
export const uniform = (nodeOrType) => {
	...
	const value =
		nodeOrType.isNode === true
			? nodeOrType.node?.value || nodeOrType.value
			: nodeOrType;
...
export const transformedNormalView = nodeImmutable(
	VarNode,
	normalView,
	"TransformedNormalView"
);
export const transformedNormalWorld = normalize(
	transformDirection(transformedNormalView, cameraViewMatrix)
);
...
export const transformedTangentView = nodeImmutable(
	VarNode,
	tangentView,
	"TransformedTangentView"
);
export const transformedTangentWorld = normalize(
	transformDirection(transformedTangentView, cameraViewMatrix)
);
...
export const transformedBitangentView = normalize(
	mul(cross(transformedNormalView, transformedTangentView), tangentGeometry.w)
);
export const transformedBitangentWorld = normalize(
	transformDirection(transformedBitangentView, cameraViewMatrix)
);
...
export const sampler = (texture) =>
	nodeObject(
		new ConvertNode(
			texture.isNode === true ? texture : new TextureNode(texture),
			"sampler"
		)
	);
