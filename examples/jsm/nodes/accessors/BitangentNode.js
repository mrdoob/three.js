...
class BitangentNode extends Node {
	...
	generate(builder) {
		...
		const crossNormalTangent = new MathNode(
			MathNode.CROSS,
			new NormalNode(scope),
			new TangentNode(scope)
		);
		const tangentW = new SplitNode(new TangentNode(TangentNode.GEOMETRY), "w");
		const vertexNode = new SplitNode(
			new OperatorNode("*", crossNormalTangent, tangentW),
			"xyz"
		);

		const outputNode = new MathNode(
			MathNode.NORMALIZE,
