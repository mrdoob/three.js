		...
		if (scope === MaterialNode.ALPHA_TEST) {
			...
			if (material.alphaMap?.isTexture === true) {
				node = new OperatorNode(
					"*",
					opacityNode,
					new MaterialReferenceNode("alphaMap", "texture")
				);
			} else {
				node = opacityNode;
			}
			...
			if (material.roughnessMap?.isTexture === true) {
				node = new OperatorNode(
					"*",
					roughnessNode,
					new SplitNode(new TextureNode(material.roughnessMap), "g")
				);
			} else {
				node = roughnessNode;
			}
			...
			if (material.metalnessMap?.isTexture === true) {
				node = new OperatorNode(
					"*",
					metalnessNode,
					new SplitNode(new TextureNode(material.metalnessMap), "b")
				);
			} else {
				node = metalnessNode;
			}
			...
			if (material.emissiveMap?.isTexture === true) {
				node = new OperatorNode(
					"*",
					emissiveNode,
					new TextureNode(material.emissiveMap)
				);
			} else {
				node = emissiveNode;
			}
