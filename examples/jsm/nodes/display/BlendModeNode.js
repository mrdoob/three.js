...
export const BurnNode = new ShaderNode(({ base, blend }) => {
	const fn = (c) =>
		cond(
			lessThan(blend[c], EPSILON),
			blend[c],
			max(sub(1.0, div(sub(1.0, base[c]), blend[c])), 0)
		);
...
export const DodgeNode = new ShaderNode(({ base, blend }) => {
	const fn = (c) =>
		cond(
			equal(blend[c], 1.0),
			blend[c],
			max(div(base[c], sub(1.0, blend[c])), 0)
		);
...
export const OverlayNode = new ShaderNode(({ base, blend }) => {
	const fn = (c) =>
		cond(
			lessThan(base[c], 0.5),
			mul(2.0, base[c], blend[c]),
			sub(1.0, mul(sub(1.0, base[c]), sub(1.0, blend[c])))
		);
