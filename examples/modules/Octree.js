...
( function ( THREE ) {
	...
	__Octree = function ( parameters ) {
		...
		this.root = parameters.root instanceof __OctreeNode ? parameters.root : new __OctreeNode( parameters );
	...
	__Octree.prototype = {
		...
		addDeferred: function ( object, options ) {
			...
			if ( object instanceof __OctreeObjectData ) {
		...
		addObjectData: function ( object, part ) {
			...
			var objectData = new __OctreeObjectData( object, part );
		...
		rebuild: function () {
			...
			for ( i = 0, l = this.objectsData.length; i < l; i ++ ) {
				...
				if ( node instanceof __OctreeNode && ! objectData.positionLast.equals( objectData.position ) ) {
		...
		setRoot: function ( root ) {
			...
			if ( root instanceof __OctreeNode ) {
	...
	__OctreeObjectData = function ( object, part ) {
	...
	__OctreeNode = function ( parameters ) {
		...
		if ( parameters.tree instanceof __Octree ) {

			this.tree = parameters.tree;

		} else if ( parameters.parent instanceof __OctreeNode !== true ) {

			parameters.root = this;

			this.tree = new __Octree( parameters );
	...
	__OctreeNode.prototype = {
		...
		updateProperties: function () {
			...
			if ( this.parent instanceof __OctreeNode ) {
		...
		addObject: function ( object ) {
			...
			} else if ( indexOctant < - 1 && this.parent instanceof __OctreeNode ) {
		...
		branch: function ( indexOctant ) {
			...
			if ( this.nodesByIndex[ indexOctant ] instanceof __OctreeNode ) {
		...
		checkMerge: function () {
			...
			while ( nodeParent.parent instanceof __OctreeNode && nodeParent.getObjectCountEnd() < this.tree.objectsThreshold ) {
		...
		checkContract: function () {
			...
			if ( this.nodesIndices.length > 0 ) {
				...
				for ( i = 0, l = this.nodesIndices.length; i < l; i ++ ) {
					...
					if ( nodeHeaviest instanceof __OctreeNode === false || nodeObjectsCount > nodeHeaviestObjectsCount ) {
				...
				if ( outsideHeaviestObjectsCount < this.tree.objectsThreshold && nodeHeaviest instanceof __OctreeNode ) {
		...
		getOctantIndex: function ( objectData ) {
			...
			if ( objectData instanceof __OctreeObjectData ) {
			...
			} else if ( objectData instanceof __OctreeNode ) {
		...
		getObjectCountStart: function () {
			...
			while ( parent instanceof __OctreeNode ) {
...
export { __Octree as Octree, __OctreeObjectData as OctreeObjectData, __OctreeNode as OctreeNode };
