...
ColorSpaceNode.Nodes = function () {
  var LinearToLinear = new THREE.FunctionNode(["vec4 LinearToLinear( in vec4 value ) {", "	return value;", "}"].join("\n"));
  ...
  var LinearToRGBE = new FunctionNode(["vec4 LinearToRGBE( in vec4 value ) {", "	float maxComponent = max( max( value.r, value.g ), value.b );", "	float fExp = clamp( ceil( log2( maxComponent ) ), -128.0, 127.0 );", "	return vec4( value.rgb / exp2( fExp ), ( fExp + 128.0 ) / 255.0 );", "}"].join("\n"));
  ...
  var cLogLuvM = new THREE.ConstNode("const mat3 cLogLuvM = mat3( 0.2209, 0.3390, 0.4184, 0.1138, 0.6780, 0.7319, 0.0102, 0.1130, 0.2969 );");
...
ColorSpaceNode.getEncodingComponents = function (encoding) {
  switch (encoding) {
    ...
    case RGBM7Encoding:
      return ['RGBM', new THREE.FloatNode(7.0).setReadonly(true)];
    ...
    case GammaEncoding:
      return ['Gamma', new THREE.ExpressionNode('float( GAMMA_FACTOR )', 'f')];
  }
};

ColorSpaceNode.prototype = Object.create(THREE.TempNode.prototype);
