...
function StandardNode() {
  Node.call(this);
  this.color = new THREE.ColorNode(0xFFFFFF);
  this.roughness = new THREE.FloatNode(1);
  this.metalness = new FloatNode(0);
}

StandardNode.prototype = Object.create(THREE.Node.prototype);
...
StandardNode.prototype.build = function (builder) {
  ...
  if (builder.isShader('vertex')) {
    ...
    builder.mergeUniform(THREE.UniformsUtils.merge([UniformsLib.fog, UniformsLib.lights]));
    ...
    builder.addParsCode(["varying vec3 vViewPosition;", "#ifndef FLAT_SHADED", "	varying vec3 vNormal;", "#endif", "#include <fog_pars_vertex>", "#include <morphtarget_pars_vertex>", "#include <skinning_pars_vertex>", "#include <shadowmap_pars_vertex>", "#include <logdepthbuf_pars_vertex>", "#include <clipping_planes_pars_vertex>"].join("\n"));
    var output = ["#include <beginnormal_vertex>", "#include <morphnormal_vertex>", "#include <skinbase_vertex>", "#include <skinnormal_vertex>", "#include <defaultnormal_vertex>", "#ifndef FLAT_SHADED", "	vNormal = normalize( transformedNormal );", "#endif", "#include <begin_vertex>"];
    ...
    var specularRoughness = new THREE.ExpressionNode('material.specularRoughness', 'f');
    var clearcoatRoughness = new ExpressionNode('material.clearcoatRoughness', 'f');
    var contextEnvironment = {
      roughness: specularRoughness,
      bias: new THREE.SpecularMIPLevelNode(specularRoughness),
    ...
    var output = ["#include <clipping_planes_fragment>", "	#include <normal_fragment_begin>", "	#include <clearcoat_normal_fragment_begin>", "	PhysicalMaterial material;", "	material.diffuseColor = vec3( 1.0 );"];
