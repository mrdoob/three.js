...
CameraNode.Nodes = function () {
  var depthColor = new THREE.FunctionNode(["float depthColor( float mNear, float mFar ) {", "	#ifdef USE_LOGDEPTHBUF_EXT", "		float depth = gl_FragDepthEXT / gl_FragCoord.w;", "	#else", "		float depth = gl_FragCoord.z / gl_FragCoord.w;", "	#endif", "	return 1.0 - smoothstep( mNear, mFar, depth );", "}"].join("\n"));
  return {
...
CameraNode.prototype = Object.create(THREE.TempNode.prototype);
...
CameraNode.prototype.generate = function (builder, output) {
  ...
  switch (this.scope) {
    ...
    case CameraNode.TO_VERTEX:
      result = 'normalize( ' + new THREE.PositionNode(PositionNode.WORLD).build(builder, 'v3') + ' - cameraPosition )';
      break;
  }
