...
function GeometryParametersPanel(editor, object) {
	...
	segmentsRow.add(
		new UIText(
			strings.getKey("sidebar/geometry/lathe_geometry/segments")
		).setWidth("90px")
	);
	...
	const phiStart = new UINumber((parameters.phiStart * 180) / Math.PI).onChange(
		update
	);

	phiStartRow.add(
		new UIText(
			strings.getKey("sidebar/geometry/lathe_geometry/phistart")
		).setWidth("90px")
	);
	...
	const phiLength = new UINumber(
		(parameters.phiLength * 180) / Math.PI
	).onChange(update);

	phiLengthRow.add(
		new UIText(
			strings.getKey("sidebar/geometry/lathe_geometry/philength")
		).setWidth("90px")
	);
	...
	pointsRow.add(
		new UIText(
			strings.getKey("sidebar/geometry/lathe_geometry/points")
		).setWidth("90px")
	);
