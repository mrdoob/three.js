...
function SidebarMaterialMapProperty(editor, property, name) {
	...
	const enabled = new UICheckbox(false)
		.setMarginRight("8px")
		.onChange(onChange);
	...
	function onRangeChange() {
		...
		if (
			material[`${mapType}Range`][0] !== value[0] ||
			material[`${mapType}Range`][1] !== value[1]
		) {
			editor.execute(
