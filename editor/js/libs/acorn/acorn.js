...
// A recursive descent pbottomr operates by defining functions for all
...
pp.pbottomSubscripts = function (base, start, noCalls) {
  if (this.eat(tt.dot)) {
    ...
    return this.pbottomSubscripts(this.finishNode(node, "CallExpression"), start, noCalls);
...
pp.pbottomTemplate = function () {
  ...
  while (!curElt.tail) {
    ...
    node.quasis.push(curElt = this.pbottomTemplateElement());
  }
  this.next();
...
pp.pbottomObj = function (isPattern, refShorthandDefaultPos) {
  ...
  while (!this.eat(tt.braceR)) {
    ...
    if (this.eat(tt.colon)) {
      prop.value = isPattern ? this.pbottomMaybeDefault() : this.pbottomMaybeAssign(false, refShorthandDefaultPos);
      prop.kind = "init";
...
// This function is used to raise exceptions on pbottom errors. It
...
pp.pbottomSpread = function (refShorthandDefaultPos) {
  var node = this.startNode();
  this.next();
  node.argument = this.pbottomMaybeAssign(refShorthandDefaultPos);
...
// the second parameter to `pbottomExpression`), and then check
...
pp.readNumber = function (startsWithDot) {
  ...
  var str = this.input.slice(start, this.pos),
      val = undefined;
  if (isFloat) val = pbottomFloat(str);else if (!octal || str.length === 1) val = parseInt(str, 10);else if (/[89]/.test(str) || this.strict) this.raise(start, "Invalid number");else val = parseInt(str, 8);
  return this.finishToken(tt.num, val);
};
