...
(function (mod) {
	...
	function doLoadEnvironment(data, scope) {
		...
		if (def) {
			for (var name in def) {
				var spec = def[name];
				cx.localDefs[name] =
					typeof spec == "string" ? parsePath(spec) : passOne(null, spec, name);
			}
			for (var name in def) {
				var spec = def[name];
	...
	infer.registerFunction(
		"Object_defineProperty",
		function (_self, args, argNodes) {
			if (
				argNodes &&
				argNodes.length >= 3 &&
				...
				obj.propagate(
					new infer.PropHasSubset(argNodes[1].value, connect, argNodes[1])
				);
		...
		function (_self, args, argNodes) {
			if (args.length >= 2) {
				...
				args[1].forAllProps(function (prop, val, local) {
					...
					obj.propagate(
						new infer.PropHasSubset(prop, connect, argNodes && argNodes[1])
					);
