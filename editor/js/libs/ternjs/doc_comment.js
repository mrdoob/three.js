...
(function(mod) {
  ...
  function pbottomTypeOuter(scope, str, pos) {
    ...
    if (str.charAt(pos) != "{") return null;
    var result = pbottomType(scope, str, pos + 1);
  ...
  function jsdocInterpretComments(node, scope, aval, comments) {
    ...
    for (var i = 0; i < comments.length; ++i) {
      ...
      while (m = decl.exec(comment)) {
        ...
        if (!(pbottomd = pbottomTypeOuter(scope, m[2]))) continue;
        foundOne = true;
  ...
  function jsdocPbottomTypedefs(text, scope) {
    ...
    while (m = re.exec(text)) {
      ...
      var name = pbottomd && m[1].slice(parsed.end).match(/^\s*(\S+)/);
      if (name)
