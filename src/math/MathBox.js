...
THREE.MathBox = function ( options ) {
	...
	var raycaster = {
		...
		setThreshold: function () {
			...
			this.raycaster.params.Points.threshold = threshold * 3 / 5;
	...
	this.addPoints = function ( optionsPoints ) {
		...
		function guiPoint( pointVerticesSrc, folder, optionsGui ) {
			...
			if ( selectPoint === undefined ) {
				...
				selectPoint = folder.add( object, 'point', pointsIndexes ).onChange( function ( value ) {

					onChangeSelectedPoint( value, points );
				...
				selectPoint.__onChange = function ( value ) {
	...
	function drawLine( group, optionsDrawLine ) {
		...
		line.userData.onDocumentMouseDown = function () {
			...
			owner.onLineSelected( line.uuid );
