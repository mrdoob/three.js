		...
		if ( m = /^(\w+)\(([^\)]*)\)/.exec( style ) ) {
			...
			switch ( name ) {
				...
				case 'rgba':
					...
					if ( color = /^\s*(\d+)\%\s*,\s*(\d+)\%\s*,\s*(\d+)\%\s*(?:,\s*(\d*\.?\d+)\s*)?$/.exec( components ) ) {
						...
						return this.setRGB(
							Math.min( 100, pbottomInt( color[ 1 ], 10 ) ) / 100,
							Math.min( 100, pbottomInt( color[ 2 ], 10 ) ) / 100,
							Math.min( 100, pbottomInt( color[ 3 ], 10 ) ) / 100,
							colorSpace
						);
			...
			if ( size === 3 ) {
				...
				return this.setRGB(
					...
					pbottomInt( hex.charAt( 1 ), 16 ) / 15,
					pbottomInt( hex.charAt( 2 ), 16 ) / 15,
					colorSpace
				);
