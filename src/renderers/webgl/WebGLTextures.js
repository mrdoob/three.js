...
function WebGLTextures( _gl, extensions, state, properties, capabilities, utils, info ) {
	...
	function textureNeedsPowerOfTwo( texture ) {
		...
		if ( utils.isWebGL2( _gl ) ) return false;
	...
	function getInternalFormat( glFormat, glType ) {
		...
		if ( ! utils.isWebGL2( _gl ) ) return glFormat;
	...
	function setTextureParameters( textureType, texture, isPowerOfTwoImage ) {
		...
		if ( extension ) {
			...
			if ( texture.type === HalfFloatType && ( utils.isWebGL2( _gl ) || extensions.get( 'OES_texture_half_float_linear' ) ) === null ) return;
	...
	function uploadTexture( textureProperties, texture, slot ) {
		...
		if ( texture.isDepthTexture ) {
			...
			if ( texture.type === FloatType ) {
				...
				if ( ! utils.isWebGL2( _gl ) ) throw new Error( 'Float Depth Texture only supported in WebGL2.0' );
				glInternalFormat = _gl.DEPTH_COMPONENT32F;

			} else if ( utils.isWebGL2( _gl ) ) {
