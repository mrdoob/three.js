...
class WebGLProgram {
	...
	constructor( renderer, cacheKey, parameters ) {

		this.programIdCount = 0;
		...
		var glVertexShader = new WebGLShader( this.gl, gl.VERTEX_SHADER, vertexGlsl );
		var glFragmentShader = new WebGLShader( this.gl, gl.FRAGMENT_SHADER, fragmentGlsl );
		...
		if ( renderer.debug.checkShaderErrors ) {
			...
			var programLog = this.gl.getProgramInfoLog( program ).trim();
			...
			if ( this.gl.getProgramParameter( program, gl.LINK_STATUS ) === false ) {
				...
				console.error( 'THREE.WebGLProgram: shader error: ', this.gl.getError(), 'gl.VALIDATE_STATUS', this.gl.getProgramParameter( program, gl.VALIDATE_STATUS ), 'gl.getProgramInfoLog', programLog, vertexErrors, fragmentErrors );
