...
class WebGLBackend extends Backend {
		...
		for (const bindGroup of bindings) {
			...
			for (const binding of bindGroup.bindings) {
				...
				if (binding.isUniformsGroup || binding.isUniformBuffer) {
					...
					// --- ANTI-UBO SUPPORT ---
					// Instead of Block Index/Binding, get individual locations for all uniforms in the group
					for (const uniform of binding.uniforms) {

						const location = gl.getUniformLocation(programGPU, uniform.name);
						if (!bindingData.programLocations) bindingData.programLocations = new WeakMap();

						let locations = bindingData.programLocations.get(programGPU);
						if (!locations) {

							locations = new Map();
							bindingData.programLocations.set(programGPU, locations);
					...
					// --- ANTI-UBO SUPPORT ---
					// Standard Uniform setup - Store location associated with this specific program
					const location = gl.getUniformLocation(programGPU, binding.name);
					if (!bindingData.programLocations) bindingData.programLocations = new WeakMap();
					bindingData.programLocations.set(programGPU, location);
					// ------------------------
		...
		for (const bindGroup of bindings) {
			...
			for (const binding of bindGroup.bindings) {
				...
				if (binding.isUniformsGroup || binding.isUniformBuffer) {
					...
					// --- ANTI-UBO SUPPORT ---
					// Bind individual uniforms in the group
					const locations = bindingData.programLocations ? bindingData.programLocations.get(state.currentProgram) : null;
					...
					if (locations) {
						...
						for (const uniform of binding.uniforms) {
							...
							if (location) {
								...
								this._setUniform(location, uniform.getValue(), uniform.getType());
					...
					// --- ANTI-UBO SUPPORT ---
					// Standard Uniform Upload - Retrieve the location for the CURRENT program
					const location = bindingData.programLocations ? bindingData.programLocations.get(state.currentProgram) : null;

					if (location) {

						this._setUniform(location, binding.getValue(), binding.getType());
	 ...
	 *
	 * @private
	 * @param {WebGLUniformLocation} location - The location.
	...
	_setUniform(location, value, type) {
		...
		if (typeof value === 'number') {
			...
			else if (type === 'uint') gl.uniform1ui(location, value);
			else if (type === 'bool') gl.uniform1i(location, value ? 1 : 0);
			...
			if (value.isColor) gl.uniform3f(location, value.r, value.g, value.b);
			else gl.uniform3f(location, value.x, value.y, value.z);
