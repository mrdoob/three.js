...
THREE.WebGLRenderer2 = function ( parameters ) {
	...
	function deleteMeshBuffers ( geometryGroup ) {
		...
		renderer.deleteBuffer( geometryGroup.__webglVertexBuffer );
		renderer.deleteBuffer( geometryGroup.__webglNormalBuffer );
		renderer.deleteBuffer( geometryGroup.__webglTangentBuffer );
		renderer.deleteBuffer( geometryGroup.__webglColorBuffer );
		renderer.deleteBuffer( geometryGroup.__webglUVBuffer );
		renderer.deleteBuffer( geometryGroup.__webglUV2Buffer );

		renderer.deleteBuffer( geometryGroup.__webglSkinIndicesBuffer );
		renderer.deleteBuffer( geometryGroup.__webglSkinWeightsBuffer );

		renderer.deleteBuffer( geometryGroup.__webglFaceBuffer );
		renderer.deleteBuffer( geometryGroup.__webglLineBuffer );
	...
	this.render = function ( scene, camera, renderTarget, forceClear ) {
		...
		if ( this.autoClear || forceClear ) {
			...
			renderer.clear( this.autoClearColor, this.autoClearDepth, this.autoClearStencil );
	...
	function renderPlugins( plugins, scene, camera ) {
		...
		for ( var i = 0, il = plugins.length; i < il; i ++ ) {
			...
			plugins[ i ].render( scene, camera, renderer.currentWidth, renderer.currentHeight );
	...
	function getTextureUnit() {
		...
		if ( textureUnit >= renderer.maxTextures ) {
			...
			console.warn( "Trying to use " + textureUnit + " texture units while this GPU supports only " + renderer.maxTextures );
