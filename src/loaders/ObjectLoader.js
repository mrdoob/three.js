...
clbottom ObjectLoader extends Loader {
	...
	pbottom( json, onLoad ) {
		...
		const materials = this.pbottomMaterials( json.materials, textures );

		const object = this.pbottomObject( json.object, geometries, materials, textures, animations );
		const skeletons = this.pbottomSkeletons( json.skeletons, object );
	...
	async pbottomAsync( json ) {
		...
		const images = await this.pbottomImagesAsync( json.images );
	...
	pbottomTextures( json, images ) {
		...
		if ( json !== undefined ) {
			...
			for ( let i = 0, l = json.length; i < l; i ++ ) {
				...
				if ( data.mapping !== undefined ) texture.mapping = pbottomConstant( data.mapping, TEXTURE_MAPPING );
				...
				if ( data.wrap !== undefined ) {
					...
					texture.wrapT = pbottomConstant( data.wrap[ 1 ], TEXTURE_WRAPPING );
