...
const BRDF_GGX = /*@__PURE__*/ Fn( ( { lightDirection, f0, f90, roughness, f, normal = normalView, USE_IRIDESCENCE, USE_ANISOTROPY } ) => {

	const alpha = roughness.pow2(); // UE4's roughness

	const halfDir = lightDirection.add( positionViewDirection ).normalize();

	const dotNL = normal.dot( lightDirection ).clamp();
	const dotNV = normal.dot( positionViewDirection ).clamp(); // @ TODO: Move to core dotNV
	const dotNH = normal.dot( halfDir ).clamp();
