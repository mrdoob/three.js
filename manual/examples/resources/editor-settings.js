(function () {
	...
	function fixSourceLinks(url, source) {
		...
		const loaderLoadRE = /(loader\.load[a-z]*\s*\(\s*)('|")(.*?)('|")/gi;
		const loaderArrayLoadRE = /(loader\.load[a-z]*\(\[)([\s\S]*?)(\])/gi;
		const loadFileRE = /(loadFile\s*\(\s*)('|")(.*?)('|")/gi;
		const threejsUrlRE =
			/(.*?)('|")([^"']*?)('|")([^'"]*?)(\/\*\s+threejs.org:\s+url\s+\*\/)/gi;
		...
		function makeArrayLinksFDed(match, prefix, arrayStr, suffix) {
			const lines = arrayStr.split(",").map((line) => {
				...
				return m ? `${m[1]}${addPrefix(m[2])}${m[3]}` : line;
			});
		...
		if (!version) {
			try {
				const res = await fetch(
					"https://raw.githubusercontent.com/mrdoob/three.js/master/package.json"
				);
