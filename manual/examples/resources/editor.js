(function () {
	...
	const webglDebugHelperScriptRE =
		/<script src="[^"]+webgl-debug-helper\.js"><\/script>/;
	...
	function fixCSSLinks(url, source) {
		...
		function addPrefix(url) {
			return url.indexOf("://") < 0 && !url.startsWith("data:")
				? `${prefix}/${url}`
				: url;
		}
	...
	async function parseHTML(url, html) {
		...
		const externalScriptRE =
			/(<!--(?:(?!-->)[\s\S])*?-->\n){0,1}<script\s+([^>]*?)(type="module"\s+)?src\s*=\s*"(.*?)"(.*?)>\s*<\/script>/gi;
		const dataScriptRE =
			/(<!--(?:(?!-->)[\s\S])*?-->\n){0,1}<script([^>]*?type="(?!module).*?".*?)>([^]*?)<\/script>/gi;
		...
		addSource(
			...
			"css",
			formatCSS(
				fixCSSLinks(url, getHTMLPart(styleRE, obj, "<style>\n${css}</style>"))
			)
		);
		addSource("html", "html", getHTMLPart(bodyRE, obj, "<body>${html}</body>"));
		const rootScript =
			getHTMLPart(inlineScriptRE, obj, "<script>${js}</script>") ||
			getHTMLPart(
		...
		html = html.replace(externalScriptRE, function (p0, p1, p2, type, p3, p4) {
			p1 = p1 || "";
			scripts.push(
				`${p1}<${kScript} ${p2}${safeStr(type)}src="${p3}"${p4}></${kScript}>`
			);
			return "";
		});
		...
		html = html.replace(
			dataScriptRE,
			function (p0, blockComments, scriptTagAttrs, content) {
				...
				dataScripts.push(
					`${blockComments}<${kScript} ${scriptTagAttrs}>${content}</${kScript}>`
				);
				return "";
			}
		);
		...
		html = html.replace(
			"</head>",
			'<script id="hackedparams">window.hackedParams = ${hackedParams}\n</script>\n</head>'
		);
		...
		function makeBlobURLForSourcesImpl(scriptInfo) {
			if (scriptInfo.blobGenerationId !== blobGeneration) {
				...
				if (scriptInfo.isWorker) {
					const extra = `self.lessonSettings = ${JSON.stringify(
						lessonSettings
					)};
...
import '${dirname(scriptInfo.fqURL)}/resources/lessons-worker-helper.js';`;
  ...
  <script match="false">self.lessonSettings = ${JSON.stringify(
		lessonSettings
	)}</script>`
		);
...
<script src="${prefix}/resources/lessons-helper.js"></script>
		...
		g.rootScriptInfo.numLinesBeforeScript = (
	...
	function getScripts(scriptInfo) {
		...
		function getScriptsImpl(scriptInfo) {
			...
			if (scriptInfo.blobGenerationId !== blobGeneration) {
				...
				scriptInfo.deps.forEach((depScriptInfo) => {
					text = text
						.split(depScriptInfo.fqURL)
						.join(`worker-${basename(depScriptInfo.fqURL)}`);
				});
	...
	function makeScriptsForWorkers(scriptInfo) {
		...
		mainScriptInfo.text = mainScriptInfo.text
			.split(`'${workerName}'`)
			.join("getWorkerBlob()");
		const html = scripts
			.map((nameText) => {
...
`;
  ...
  `;
		...
		elem.querySelector("input[name=css]").value =
...
`;
		...
		const tags = lessonEditorSettings.tags
...
`;
		...
		const files = scripts.reduce(
			(files, { name, text: content }) => {
				files[name] = { content };
				return files;
			},
			{
...
*/
	...
	function setupEditor() {
		...
		document
			.querySelector(".button-codepen")
			.addEventListener("click", closeExport(openInCodepen));
		document
			.querySelector(".button-jsfiddle")
			.addEventListener("click", closeExport(openInJSFiddle));
		document
			.querySelector(".button-jsgist")
			.addEventListener("click", closeExport(openInJSGist));
		document
			.querySelector(".button-stackoverflow")
			.addEventListener("click", closeExport(openInStackOverflow));
		document
			.querySelector(".button-codesandbox")
			.addEventListener("click", closeExport(openInCodeSandbox));
	...
	function getEditorNdxByBlobUrl(type, url) {
		return htmlParts[type].sources.findIndex(
			(source) => source.scriptInfo.blobUrl === url
		);
	}
	...
	function getActualLineNumberAndMoveTo(url, lineNo, colNo) {
		...
		const scriptInfo = Object.values(g.scriptInfos).find(
			(scriptInfo) => scriptInfo.blobUrl === url
		);
		if (scriptInfo) {
	...
	function start() {
		...
		const isSmallish = window.navigator.userAgent.match(
			/Android|iPhone|iPod|Windows Phone/i
		);
		...
		if (isEdge || isSmallish || parentQuery.editor === "false") {
			...
			applySubstitutions();
			require.config({
				paths: { vs: "/manual/3rdparty/monaco-editor/min/vs" },
			});
