...

{
	...
	function scaleCube(zOffset) {
		...
		geometry.applyMatrix4(
			new THREE.Matrix4().makeTranslation(0, 0, zOffset * size)
		);
		...
		return {
			obj3D: root,
			update: (time) => {
				const s = THREE.MathUtils.lerp(0.5, 2, Math.sin(time) * 0.5 + 0.5);
	...
	threejsLessonUtils.addDiagrams({
		...
		lonLatPos: {
			create(info) {
				...
				gui
					.add(new DegRadHelper(lonHelper.rotation, "y"), "value", -180, 180)
					.name("lonHelper x rotation");
				gui
					.add(new DegRadHelper(latHelper.rotation, "x"), "value", -90, 90)
					.name("latHelper y rotation");
