...

{
	...
	const filterTextureInfo = loadTextureAndPromise(
		"/manual/resources/images/mip-example.png"
	);
	...
	function lowResCube(scale, pixelSize = 16) {
		...
		return {
			...
			update(time, renderInfo) {
				const { width, height, scene, camera, renderer, pixelRatio } =
					renderInfo;
	...
	threejsLessonUtils.addDiagrams({
		...
		filterCubeSmall: {
			create(info) {
				return lowResCube(0.1, info.renderInfo.pixelRatio);
			},
		},
			...
			async create(props) {
				...
				renderInfo.elem.addEventListener("click", () => {
					for (const meshInfo of meshInfos) {
						...
						material.map =
							material.map === copyTexture ? mipTexture : copyTexture;
					}
				});
	...
	[...document.querySelectorAll("[data-texture-diagram]")].forEach(
		createTextureDiagram
	);
}
