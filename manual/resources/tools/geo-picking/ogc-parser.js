...
const assert = {
	strictEqual(actual, expected, ...args) {
		...
		if (actual !== expected) {
			throw new Error(
				`${actual} (actual) should equal ${expected} (expected): ${[
					...args,
				].join(" ")}`
			);
		}
	...
	notStrictEqual(actual, expected, ...args) {
		...
		if (actual === expected) {
			throw new Error(
				`${actual} (actual) should NOT equal ${expected} (expected): ${[
					...args,
				].join(" ")}`
			);
		}
...
function parse(buf) {
  ...
  */
	...
	const getUint32 = () => {
		const v = dataView.getUint32(cursor, littleEndian);
		cursor += 4;
		return v;
	};
