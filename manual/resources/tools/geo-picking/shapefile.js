...
(function (global, factory) {
	...
	function slice(source) {
		return typeof source.slice === "function"
			? source
			: new SliceSource(
					typeof source.read === "function" ? source : source.getReader()
			  );
	}
	...
	var shp_read = function () {
		...
		return that._source.slice(12).then(function (array) {
			...
			function read() {
				...
				return length < 0 || (type && type !== that._type)
					? skip()
					: that._source.slice(length).then(function (chunk) {
							return {
								done: false,
								value: type
									? that._parse(view(concat$1(array.slice(8), chunk)))
									: null,
							};
					  });
			}
	...
	function open(shp$$1, dbf$$1, options) {
		...
		if (typeof shp$$1 === "string") {
			...
			if (dbf$$1 === undefined)
				dbf$$1 = path(shp$$1.substring(0, shp$$1.length - 4) + ".dbf").catch(
					function () {}
				);
			shp$$1 = path(shp$$1);
