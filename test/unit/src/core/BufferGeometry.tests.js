...
export default QUnit.module( 'Core', () => {
	...
	QUnit.module( 'BufferGeometry', () => {
		...
		QUnit.test( 'applyMatrix4 - 2D', ( assert ) => {

			const geometry = new BufferGeometry();
			geometry.setAttribute( 'position', new BufferAttribute( new Float32Array( 4 ), 2 ) );
			...
			const position = geometry.attributes.position.array;
			const m = matrix.elements;
			assert.ok( position[ 0 ] === m[ 12 ] && position[ 1 ] === m[ 13 ], 'position was extracted from 2d matrix' );
			assert.ok( position[ 2 ] === m[ 12 ] && position[ 3 ] === m[ 13 ], 'position was extracted from 2d matrix twice' );
			assert.ok( geometry.attributes.position.version === 1, 'version was increased during update' );
