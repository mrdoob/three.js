...
export default QUnit.module( 'Core', () => {
    ...
    QUnit.module( 'EventDispatcher', () => {
        ...
        QUnit.test( 'addEventListener', ( assert ) => {
            ...
            assert.ok( eventDispatcher._listeners.get( 'anyType' ).size === 1, 'listener with unknown type was added' );
            assert.ok( eventDispatcher._listeners.get( 'anyType' ).has( listener ), 'listener with unknown type was added' );

            eventDispatcher.addEventListener( 'anyType', listener );

            assert.ok( eventDispatcher._listeners.get( 'anyType' ).size === 1, 'can\'t add one listener twice to same type' );
            assert.ok( eventDispatcher._listeners.get( 'anyType' ).has( listener ), 'listener is still there' );
