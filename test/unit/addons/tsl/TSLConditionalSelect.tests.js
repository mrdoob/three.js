...
export default QUnit.module( 'TSL', () => {
	...
	QUnit.module( 'Conditional (select)', () => {
		...
		gpuTest( 'the chained .select() method form also selects per-component for a vector condition', ( { assert } ) => {

			const cond = vec3( - 1, 2, - 3 ).toVar().greaterThan( 0 );
			const result = cond.select( vec3( 10, 20, 30 ), vec3( 1, 2, 3 ) );

			assert.eq( result, vec3( 1, 20, 3 ), 'chained select() takes the "if" value before the "else" value' );
