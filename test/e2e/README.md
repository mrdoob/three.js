...
Time stays frozen while frames advance. The render timeout scales with the frame count.
Captures using multiple frames fail if the requested frame count is not reached. Update the
count when an example's default bake settings change.
