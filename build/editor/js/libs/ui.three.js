...
class UITexture extends UISpan {
  ...
  setValue(texture) {
    ...
    const context = canvas.getContext('2d'); // Seems like context can be null if the canvas is not visible
...
class UIOutliner extends UIDiv {
  constructor(editor) {
    ...
    this.setContextMenuOptions = function (options) {
      this.contextMenu.setOptions(options);
      return this;
    };

    this.onContextMenuChange = function (callback) {
    ...
    const scope = this; // hack

    this.scene = editor.scene; // Prevent native scroll behavior
  ...
  setValue(value) {
    for (let i = 0; i < this.options.length; i++) {
      ...
      if (element.value === value) {
        element.classList.add('active'); // scroll into view
...
class UIPoints extends UISpan {
  constructor() {
    ...
    this.pointsUI = [];
    this.lastPointIdx = 0;
    this.onChangeCallback = null; // TODO Remove this bind() stuff
