...
(function (root, mod) {
  if (typeof exports == "object" && typeof module == "object") // CommonJS
    return mod(exports, require("./infer"), require("./signal"), require("acorn"), require("acorn/dist/walk"));
  ...
  function resolveFile(srv, localFiles, name) {
    ...
    if (file.type == "full") return srv.findFile(file.name); // This is a partial file
    ...
    infer.withContext(srv.cx, function () {
      ...
      infer.analyze(file.ast, file.name, scope, srv.passes); // This is a kludge to tie together the function types (if any)
  ...
  function findTypeAt(srv, query, file) {
    ...
    if (expr) {
      if (expr.node.type == "Identifier") exprName = expr.node.name;else if (expr.node.type == "MemberExpression" && !expr.node.computed) exprName = expr.node.property.name;
    }
