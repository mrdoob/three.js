...
(function (mod) {
  ...
  function passTwo(base, spec, path) {
    ...
    for (var name in spec) if (hop(spec, name) && name.charCodeAt(0) != 33) {
      ...
      if (typeof inner == "string") {
        ...
        if (!isSimpleAnnotation(inner)) passTwo(known.getObjType(), inner, innerPath);else if (known.isEmpty()) parseType(inner["!type"], innerPath, null, true).propagate(known);else continue;
  ...
  exports.parse = function (data, origin, path) {
    ...
    try {
      if (typeof data == "string") return parseType(data, path);else return passTwo(passOne(null, data, path), data, path);
    } finally {
  ...
  var PropSpec = infer.constraint("target", {
    addType: function (tp) {
      ...
      if (tp.hasProp("value")) tp.getProp("value").propagate(this.target);else if (tp.hasProp("get")) tp.getProp("get").propagate(new infer.IsCallee(infer.ANull, [], null, this.target));
    }
  });
  ...
  var IsBound = infer.constraint("self, args, target", {
    addType: function (tp) {
      ...
      this.target.addType(new infer.Fn(tp.name, infer.ANull, tp.args.slice(this.args.length), tp.argNames.slice(this.args.length), tp.retval));
