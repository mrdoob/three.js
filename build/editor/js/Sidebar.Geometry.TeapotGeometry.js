...
function GeometryParametersPanel(signals, object) {
  ...
  container.add(segmentsRow); // bottom
  ...
  bodyRow.add(body);
  container.add(bodyRow); // fitted lid
  ...
  container.add(fitLidRow); // blinn-sized
  ...
  function update() {
    ...
    object.geometry = new TeapotGeometry(size.getValue(), segments.getValue(), bottom.getValue(), lid.getValue(), body.getValue(), fitLid.getValue(), blinn.getValue());
