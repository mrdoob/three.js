...
function SidebarMaterialMapProperty(editor, property, name) {
  ...
  function onChange() {
    ...
    if (material[property] !== newMap) {
      ...
      editor.execute(new SetMaterialMapCommand(editor, object, property, newMap, 0
  ...
  function onIntensityChange() {
    if (material[`${property}Intensity`] !== intensity.getValue()) {
      editor.execute(new SetMaterialValueCommand(editor, object, `${property}Intensity`, intensity.getValue(), 0
      /* TODO: currentMaterialSlot */
      ));
    }
  }
  ...
  function onScaleChange() {
    if (material[`${mapType}Scale`] !== scale.getValue()) {
      editor.execute(new SetMaterialValueCommand(editor, object, `${mapType}Scale`, scale.getValue(), 0
      /* TODO: currentMaterialSlot */
      ));
    }
  }
  ...
  function onScaleXYChange() {
    ...
    if (material[`${mapType}Scale`].x !== value[0] || material[`${mapType}Scale`].y !== value[1]) {
      editor.execute(new SetMaterialVectorCommand(editor, object, `${mapType}Scale`, value, 0
      /* TODOL currentMaterialSlot */
      ));
    }
  }
