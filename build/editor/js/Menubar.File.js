...
function MenubarFile(editor) {
  ...
  options.add(option); // Export Object
  ...
  options.add(option); // Export Scene
  ...
  options.add(new UIHorizontalRule()); // Export DAE
  ...
  option.onClick(async function () {
    const {
      ColladaExporter
    } = await import('../../examples/jsm/exporters/ColladaExporter.js');
    const exporter = new ColladaExporter();
    exporter.parse(editor.scene, function (result) {
      saveString(result.data, 'scene.dae');
    });
  });
  options.add(option); // Export DRC
  ...
  option.onClick(async function () {
    ...
    const {
      DRACOExporter
    } = await import('../../examples/jsm/exporters/DRACOExporter.js');
  ...
  option.onClick(async function () {
    ...
    const {
      GLTFExporter
    } = await import('../../examples/jsm/exporters/GLTFExporter.js');
  ...
  options.add(option); // Export GLTF
  ...
  option.onClick(async function () {
    ...
    const {
      OBJExporter
    } = await import('../../examples/jsm/exporters/OBJExporter.js');
    const exporter = new OBJExporter();
    saveString(exporter.parse(object), 'model.obj');
  });
  options.add(option); // Export PLY (ASCII)
  ...
  option.onClick(async function () {
    const {
      PLYExporter
    } = await import('../../examples/jsm/exporters/PLYExporter.js');
    const exporter = new PLYExporter();
    exporter.parse(editor.scene, function (result) {
      saveArrayBuffer(result, 'model.ply');
    });
  });
  options.add(option); // Export PLY (Binary)
  ...
  options.add(option); // Export STL (ASCII)
  ...
  option.onClick(async function () {
    const {
      STLExporter
    } = await import('../../examples/jsm/exporters/STLExporter.js');
    const exporter = new STLExporter();
    saveString(exporter.parse(editor.scene), 'model.stl');
  });
  options.add(option); // Export STL (Binary)
  ...
  options.add(option); // Export USDZ
  ...
  option.onClick(async function () {
    const {
      USDZExporter
    } = await import('../../examples/jsm/exporters/USDZExporter.js');
    const exporter = new USDZExporter();
    saveArrayBuffer(await exporter.parse(editor.scene, {
      binary: true
    }), 'model.usdz');
  });
  options.add(option); //

  options.add(new UIHorizontalRule()); // Publish
  ...
  option.onClick(function () {
    ...
    loader.load('js/libs/app/index.html', function (content) {
      ...
      if (config.getKey('project/editable')) {
        editButton = ['			let button = document.createElement( \'a\' );', '			button.href = \'https://threejs.org/editor/#file=\' + location.href.split( \'/\' ).slice( 0, - 1 ).join( \'/\' ) + \'/app.json\';', '			button.style.cssText = \'position: absolute; bottom: 20px; right: 20px; padding: 10px 16px; color: #fff; border: 1px solid #fff; border-radius: 20px; text-decoration: none;\';', '			button.target = \'_blank\';', '			button.textContent = \'EDIT\';', '			document.body.appendChild( button );'].join('\n');
      }
