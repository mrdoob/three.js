...
function GeometryParametersPanel(editor, object) {
  ...
  const parameters = geometry.parameters; // innerRadius
  ...
  container.add(innerRadiusRow); // outerRadius
  ...
  container.add(outerRadiusRow); // thetaSegments
  ...
  container.add(thetaSegmentsRow); // phiSegments
  ...
  container.add(phiSegmentsRow); // thetaStart
  ...
  function update() {
    editor.execute(new SetGeometryCommand(editor, object, new THREE.RingGeometry(innerRadius.getValue(), outerRadius.getValue(), thetaSegments.getValue(), phiSegments.getValue(), thetaStart.getValue() * THREE.MathUtils.DEG2RAD, thetaLength.getValue() * THREE.MathUtils.DEG2RAD)));
  }
