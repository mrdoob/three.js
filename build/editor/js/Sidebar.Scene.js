...
function SidebarScene(editor) {
  ...
  container.setPaddingTop('20px'); // outliner
  ...
  container.add(new UIBreak()); // background
  ...
  container.add(fogTypeRow); // fog color
  ...
  function refreshUI() {
    ...
    const options = []; // options.push( buildOption( camera, false ) );
    ...
    (function addObjects(objects, pad) {
      for (let i = 0, l = objects.length; i < l; i++) {
        ...
        if (!['OrthographicCamera', 'PerspectiveCamera', 'AmbientLight', 'DirectionalLight'].includes(object.type) && !object.name.includes('label-for-')) {
