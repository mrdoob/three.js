...
function Viewport(editor) {
  ...
  let showSceneHelpers = true; // helpers
  ...
  grid.add(grid2);
  grid.rotateX(Math.PI / 2); // additioanl axis lines for usability considerations
  ...
  sceneHelpers.add(transformControls); // object picking

  const raycaster = new THREE.Raycaster();
  const mouse = new THREE.Vector2(); // events
  ...
  container.dom.addEventListener('dblclick', onDoubleClick); // controls need to be added *after* main logic,
  ...
  viewHelper.controls = controls; // signals
  ...
  signals.cameraResetted.add(updateAspectRatio); // animations
  ...
  function animate() {
    ...
    let needsUpdate = false; // Animations
  ...
  function render() {
    startTime = performance.now(); // Adding/removing grid to scene so materials with depthWrite false
