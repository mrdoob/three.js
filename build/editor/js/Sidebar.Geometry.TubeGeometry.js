...
function GeometryParametersPanel(editor, object) {
  ...
  const parameters = geometry.parameters; // points
  ...
  container.add(radialSegmentsRow); // closed
  ...
  container.add(closedRow); // curveType

  const curveTypeRow = new UIRow();
  const curveType = new UISelect().setOptions({
  ...
  container.add(curveTypeRow); // tension
  ...
  function update() {
    ...
    editor.execute(new SetGeometryCommand(editor, object, new THREE.TubeGeometry(new THREE.CatmullRomCurve3(points.getValue(), closed.getValue(), curveType.getValue(), tension.getValue()), tubularSegments.getValue(), radius.getValue(), radialSegments.getValue(), closed.getValue())));
  }
