...
function SidebarGeometry(editor) {
  ...
  let currentGeometryType = null; // Actions
  ...
  */
  ...
  container.add(geometryTypeRow); // uuid
  ...
  container.add(geometryUUIDRow); // name
  ...
  container.add(geometryNameRow); // parameters

  const parameters = new UISpan();
  container.add(parameters); // buffergeometry

  container.add(new SidebarGeometryBufferGeometry(editor)); // Size
  ...
  container.add(geometryBoundingBoxRow); // Helpers
    ...
    if (object && object.geometry) {
      ...
      if (currentGeometryType !== geometry.type) {
        ...
        if (geometry.type === 'BufferGeometry' || geometry.type === 'Geometry') {
          ...
          } = await import(`./Sidebar.Geometry.${geometry.type}.js`);
