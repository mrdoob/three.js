...
function GeometryParametersPanel(editor, object) {
  ...
  const parameters = geometry.parameters; // segments
  ...
  container.add(segmentsRow); // phiStart
  ...
  container.add(phiLengthRow); // points
  ...
  function update() {
    editor.execute(new SetGeometryCommand(editor, object, new THREE.LatheGeometry(points.getValue(), segments.getValue(), phiStart.getValue() / 180 * Math.PI, phiLength.getValue() / 180 * Math.PI)));
  }
