...
function GeometryParametersPanel(editor, object) {
  ...
  options.bevelSegments = options.bevelSegments !== undefined ? options.bevelSegments : 3; // curveSegments
  ...
  container.add(curveSegmentsRow); // steps
  ...
  depthRow.add(depth);
  container.add(depthRow); // enabled
  ...
  if (options.bevelEnabled === true) {
    ...
    container.add(thicknessRow); // size
    ...
    container.add(offsetRow); // segments
  ...
  function toShape() {
    editor.execute(new SetGeometryCommand(editor, object, new THREE.ShapeGeometry(parameters.shapes, options.curveSegments)));
  }
