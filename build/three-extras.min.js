...
void 0,q=b[g[k]].x,s=b[g[k]].y,m=b[g[l]].x,u=b[g[l]].y,v=b[g[n]].x,t=b[g[n]].y;if(1E-10>(m-q)*(t-s)-(u-s)*(v-q))r=!1;else{var x=void 0,w=void 0,z=void 0,B=void 0,A=void 0,G=void 0,F=void 0,K=void 0,H=void 0,N=void 0,H=K=F=y=D=void 0,x=v-m,w=t-u,z=q-v,B=s-t,A=m-q,G=u-s;for(r=0;r<e;r++)if(D=b[g[r]].x,y=b[g[r]].y,!(D===q&&y===s||D===m&&y===u||D===v&&y===t)&&(F=D-q,K=y-s,H=D-m,N=y-u,D-=v,y-=t,H=x*N-w*H,F=A*K-G*F,K=z*y-B*D,-1E-10<=H&&-1E-10<=K&&-1E-10<=F)){r=!1;break a}r=!0}}if(r){f.push([b[g[k]],b[g[l]],
...
d=k.length-1;f=b-1;0>f&&(f=d);g=b+1;g>d&&(g=0);return(d=e(k[b],k[f],k[g],h[a]))?!0:!1}function f(a,b){var c,e;for(c=0;c<h.length;c++)if(e=c+1,e%=h.length,e=d(a,b,h[c],h[e],!0),0<e.length)return!0;return!1}function g(a,c){var e,f,h,k;for(e=0;e<l.length;e++)for(f=b[l[e]],h=0;h<f.length;h++)if(k=h+1,k%=f.length,k=d(a,c,f[h],f[k],!0),0<k.length)return!0;return!1}var h=a.concat(),k,l=[],n,p,w,z,B,A=[],G,F,K,H=0;for(n=b.length;H<n;H++)l.push(H);G=0;for(var N=2*l.length;0<l.length;){N--;if(0>N){console.log("Infinite Loop! Holes left:"+
l.length+", Probably Hole outside Shape!");break}for(p=G;p<h.length;p++){w=h[p];n=-1;for(H=0;H<l.length;H++)if(z=l[H],B=w.x+":"+w.y+":"+z,void 0===A[B]){k=b[z];for(F=0;F<k.length;F++)if(z=k[F],c(p,F)&&!f(w,z)&&!g(w,z)){n=F;l.splice(H,1);G=h.slice(0,p+1);z=h.slice(p);F=k.slice(n);K=k.slice(0,n+1);h=G.concat(F).concat(K).concat(z);G=p;break}if(0<=n)break;A[B]=!0}if(0<=n)break}}return h}(b,a);var p=THREE.FontUtils.Triangulate(f,!1);f=0;for(g=p.length;f<g;f++)for(k=p[f],h=0;3>h;h++)l=k[h].x+":"+k[h].y,
...
(a=!0):d(f)==d(h)&&(a=!0),a?(c=-f,a=e,e=Math.sqrt(k)):(c=e,a=f,e=Math.sqrt(k/2));return new THREE.Vector2(c/e,a/e)}function e(c,d){var e,f;for(C=c.length;0<=--C;){e=C;f=C-1;0>f&&(f=c.length-1);for(var g=0,h=q+2*n,g=0;g<h;g++){var k=P*g,l=P*(g+1),m=d+e+k,k=d+f+k,p=d+f+l,l=d+e+l,r=c,s=g,u=h,v=e,x=f,m=m+K,k=k+K,p=p+K,l=l+K;F.faces.push(new THREE.Face3(m,k,l,null,null,t));F.faces.push(new THREE.Face3(k,p,l,null,null,t));m=D.generateSideWallUV(F,b,r,a,m,k,p,l,s,u,v,x);F.faceVertexUvs[0].push([m[0],m[1],
...
U.push(Q);T=T.concat(Q)}for(E=0;E<n;E++){J=E/n;R=k*(1-J);I=l*Math.sin(J*Math.PI/2);C=0;for(J=L.length;C<J;C++)M=c(L[C],s[C],I),f(M.x,M.y,-R);A=0;for(G=H.length;A<G;A++)for(B=H[A],Q=U[A],C=0,J=B.length;C<J;C++)M=c(B[C],Q[C],I),f(M.x,M.y,-R)}I=l;for(C=0;C<P;C++)M=p?c(r[C],T[C],I):r[C],u?(w.copy(y.normals[0]).multiplyScalar(M.x),x.copy(y.binormals[0]).multiplyScalar(M.y),z.copy(m[0]).add(w).add(x),f(z.x,z.y,z.z)):f(M.x,M.y,0);for(J=1;J<=q;J++)for(C=0;C<P;C++)M=p?c(r[C],T[C],I):r[C],u?(w.copy(y.normals[J]).multiplyScalar(M.x),
x.copy(y.binormals[J]).multiplyScalar(M.y),z.copy(m[J]).add(w).add(x),f(z.x,z.y,z.z)):f(M.x,M.y,h/q*J);for(E=n-1;0<=E;E--){J=E/n;R=k*(1-J);I=l*Math.sin(J*Math.PI/2);C=0;for(J=L.length;C<J;C++)M=c(L[C],s[C],I),f(M.x,M.y,h+R);A=0;for(G=H.length;A<G;A++)for(B=H[A],Q=U[A],C=0,J=B.length;C<J;C++)M=c(B[C],Q[C],I),u?f(M.x,M.y+m[q-1].y,m[q-1].x+R):f(M.x,M.y,h+R)}(function(){if(p){var a;a=0*P;for(C=0;C<S;C++)O=N[C],g(O[2]+a,O[1]+a,O[0]+a,!0);a=q+2*n;a*=P;for(C=0;C<S;C++)O=N[C],g(O[0]+a,O[1]+a,O[2]+a,!1)}else{for(C=
...
G.y=A.y*w+w,n||0<G.x&&G.x<e&&0<G.y&&G.y<f)for(a.activeTexture(a.TEXTURE1),a.bindTexture(a.TEXTURE_2D,k),a.copyTexImage2D(a.TEXTURE_2D,0,a.RGB,G.x-8,G.y-8,16,16,0),a.uniform1i(F.renderType,0),a.uniform2f(F.scale,B.x,B.y),a.uniform3f(F.screenPosition,A.x,A.y,A.z),a.disable(a.BLEND),a.enable(a.DEPTH_TEST),a.drawElements(a.TRIANGLES,6,a.UNSIGNED_SHORT,0),a.activeTexture(a.TEXTURE0),a.bindTexture(a.TEXTURE_2D,l),a.copyTexImage2D(a.TEXTURE_2D,0,a.RGBA,G.x-8,G.y-8,16,16,0),a.uniform1i(F.renderType,1),a.disable(a.DEPTH_TEST),
