...
if(q&&q[n]&&s&&s[n]){if(0<this.timeScale&&s.time<=this.currentTime||0>this.timeScale&&q.time>=this.currentTime){q=this.data.hierarchy[e].keys[0];for(s=this.getNextKeyWith(n,e,1);s.time<this.currentTime&&s.index>q.index;)q=s,s=this.getNextKeyWith(n,e,s.index+1);l.prevKey[n]=q;l.nextKey[n]=s}k.matrixAutoUpdate=!0;k.matrixWorldNeedsUpdate=!0;var p=(this.currentTime-q.time)/(s.time-q.time),w=q[n],y=s[n];0>p&&(p=0);1<p&&(p=1);if("pos"===n)if(this.interpolationType===THREE.AnimationHandler.LINEAR)b.x=w[0]+
(y[0]-w[0])*p,b.y=w[1]+(y[1]-w[1])*p,b.z=w[2]+(y[2]-w[2])*p,q=this.weight/(this.weight+m.positionWeight),k.position.lerp(b,q),m.positionWeight+=this.weight;else{if(this.interpolationType===THREE.AnimationHandler.CATMULLROM||this.interpolationType===THREE.AnimationHandler.CATMULLROM_FORWARD)a[0]=this.getPrevKeyWith("pos",e,q.index-1).pos,a[1]=w,a[2]=y,a[3]=this.getNextKeyWith("pos",e,s.index+1).pos,p=0.33*p+0.33,s=f(a,p),q=this.weight/(this.weight+m.positionWeight),m.positionWeight+=this.weight,n=
k.position,n.x+=(s[0]-n.x)*q,n.y+=(s[1]-n.y)*q,n.z+=(s[2]-n.z)*q,this.interpolationType===THREE.AnimationHandler.CATMULLROM_FORWARD&&(p=f(a,1.01*p),c.set(p[0],p[1],p[2]),c.sub(n),c.y=0,c.normalize(),p=Math.atan2(c.x,c.z),k.rotation.set(0,p,0))}else"rot"===n?(THREE.Quaternion.slerp(w,y,d,p),0===m.quaternionWeight?(k.quaternion.copy(d),m.quaternionWeight=this.weight):(q=this.weight/(this.weight+m.quaternionWeight),THREE.Quaternion.slerp(k.quaternion,d,k.quaternion,q),m.quaternionWeight+=this.weight)):
"scl"===n&&(b.x=w[0]+(y[0]-w[0])*p,b.y=w[1]+(y[1]-w[1])*p,b.z=w[2]+(y[2]-w[2])*p,q=this.weight/(this.weight+m.scaleWeight),k.scale.lerp(b,q),m.scaleWeight+=this.weight)}}return!0}}}();
