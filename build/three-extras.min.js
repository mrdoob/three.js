...
c.matrix);for(var c=c.animationCache.animations[this.data.name],d=0;3>d;d++){var f=this.keyTypes[d],e=this.data.hierarchy[a].keys[0],g=this.getNextKeyWith(f,a,1);if(e&&e[f]&&g&&g[f]){for(;g.time<this.currentTime&&g.index>e.index;)e=g,g=this.getNextKeyWith(f,a,g.index+1);c.prevKey[f]=e;c.nextKey[f]=g}}}};
...
(-3*(b-c)-2*a-d)*f+a*e+b};return function(e){if(!1!==this.isPlaying&&(this.currentTime+=e*this.timeScale,0!==this.weight)){e=this.data.length;if(this.currentTime>e||0>this.currentTime)this.loop?(this.currentTime%=e,0>this.currentTime&&(this.currentTime+=e),this.reset()):this.stop();e=0;for(var h=this.hierarchy.length;e<h;e++)for(var k=this.hierarchy[e],l=k.animationCache.animations[this.data.name],m=k.animationCache.blending,q=0;3>q;q++){var n=this.keyTypes[q],r=l.prevKey[n],s=l.nextKey[n];if(r&&
r[n]&&s&&s[n]){if(0<this.timeScale&&s.time<=this.currentTime||0>this.timeScale&&r.time>=this.currentTime){r=this.data.hierarchy[e].keys[0];for(s=this.getNextKeyWith(n,e,1);s.time<this.currentTime&&s.index>r.index;)r=s,s=this.getNextKeyWith(n,e,s.index+1);l.prevKey[n]=r;l.nextKey[n]=s}var p=(this.currentTime-r.time)/(s.time-r.time),t=r[n],u=s[n];0>p&&(p=0);1<p&&(p=1);if("pos"===n)if(this.interpolationType===THREE.AnimationHandler.LINEAR)c.x=t[0]+(u[0]-t[0])*p,c.y=t[1]+(u[1]-t[1])*p,c.z=t[2]+(u[2]-
t[2])*p,r=this.weight/(this.weight+m.positionWeight),k.position.lerp(c,r),m.positionWeight+=this.weight;else{if(this.interpolationType===THREE.AnimationHandler.CATMULLROM||this.interpolationType===THREE.AnimationHandler.CATMULLROM_FORWARD)a[0]=this.getPrevKeyWith("pos",e,r.index-1).pos,a[1]=t,a[2]=u,a[3]=this.getNextKeyWith("pos",e,s.index+1).pos,p=.33*p+.33,s=f(a,p),r=this.weight/(this.weight+m.positionWeight),m.positionWeight+=this.weight,n=k.position,n.x+=(s[0]-n.x)*r,n.y+=(s[1]-n.y)*r,n.z+=(s[2]-
n.z)*r,this.interpolationType===THREE.AnimationHandler.CATMULLROM_FORWARD&&(p=f(a,1.01*p),b.set(p[0],p[1],p[2]),b.sub(n),b.y=0,b.normalize(),p=Math.atan2(b.x,b.z),k.rotation.set(0,p,0))}else"rot"===n?(THREE.Quaternion.slerp(t,u,d,p),0===m.quaternionWeight?(k.quaternion.copy(d),m.quaternionWeight=this.weight):(r=this.weight/(this.weight+m.quaternionWeight),THREE.Quaternion.slerp(k.quaternion,d,k.quaternion,r),m.quaternionWeight+=this.weight)):"scl"===n&&(c.x=t[0]+(u[0]-t[0])*p,c.y=t[1]+(u[1]-t[1])*
p,c.z=t[2]+(u[2]-t[2])*p,r=this.weight/(this.weight+m.scaleWeight),k.scale.lerp(c,r),m.scaleWeight+=this.weight)}}return!0}}}();
...
THREE.CameraHelper.prototype.update=function(){var a,b,c=new THREE.Vector3,d=new THREE.Camera,f=function(e,f,h,k){c.set(f,h,k).unproject(d);e=b[e];if(void 0!==e)for(f=0,h=e.length;f<h;f++)a.vertices[e[f]].copy(c)};return function(){a=this.geometry;b=this.pointMap;d.projectionMatrix.copy(this.camera.projectionMatrix);f("c",0,0,-1.00001);f("t",0,0,1);f("n1",-1,-1,-1.00001);f("n2",1,-1,-1.00001);f("n3",-1,1,-1.00001);f("n4",1,1,-1.00001);f("f1",-1,-1,1);f("f2",1,-1,1);f("f3",-1,1,1);f("f4",1,1,1);f("u1",
.7,1.1,-1.00001);f("u2",-.7,1.1,-1.00001);f("u3",0,2,-1.00001);f("cf1",-1,0,1);f("cf2",1,0,1);f("cf3",0,-1,1);f("cf4",0,1,1);f("cn1",-1,0,-1.00001);f("cn2",1,0,-1.00001);f("cn3",0,-1,-1.00001);f("cn4",0,1,-1.00001);a.verticesNeedUpdate=!0}}();
