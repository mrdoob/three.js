...
if(r&&r[n]&&s&&s[n]){if(0<this.timeScale&&s.time<=this.currentTime||0>this.timeScale&&r.time>=this.currentTime){r=this.data.hierarchy[e].keys[0];for(s=this.getNextKeyWith(n,e,1);s.time<this.currentTime&&s.index>r.index;)r=s,s=this.getNextKeyWith(n,e,s.index+1);l.prevKey[n]=r;l.nextKey[n]=s}k.matrixAutoUpdate=!0;k.matrixWorldNeedsUpdate=!0;var p=(this.currentTime-r.time)/(s.time-r.time),t=r[n],u=s[n];0>p&&(p=0);1<p&&(p=1);if("pos"===n)if(this.interpolationType===THREE.AnimationHandler.LINEAR)c.x=t[0]+
(u[0]-t[0])*p,c.y=t[1]+(u[1]-t[1])*p,c.z=t[2]+(u[2]-t[2])*p,r=this.weight/(this.weight+m.positionWeight),k.position.lerp(c,r),m.positionWeight+=this.weight;else{if(this.interpolationType===THREE.AnimationHandler.CATMULLROM||this.interpolationType===THREE.AnimationHandler.CATMULLROM_FORWARD)a[0]=this.getPrevKeyWith("pos",e,r.index-1).pos,a[1]=t,a[2]=u,a[3]=this.getNextKeyWith("pos",e,s.index+1).pos,p=.33*p+.33,s=f(a,p),r=this.weight/(this.weight+m.positionWeight),m.positionWeight+=this.weight,n=k.position,
n.x+=(s[0]-n.x)*r,n.y+=(s[1]-n.y)*r,n.z+=(s[2]-n.z)*r,this.interpolationType===THREE.AnimationHandler.CATMULLROM_FORWARD&&(p=f(a,1.01*p),b.set(p[0],p[1],p[2]),b.sub(n),b.y=0,b.normalize(),p=Math.atan2(b.x,b.z),k.rotation.set(0,p,0))}else"rot"===n?(THREE.Quaternion.slerp(t,u,d,p),0===m.quaternionWeight?(k.quaternion.copy(d),m.quaternionWeight=this.weight):(r=this.weight/(this.weight+m.quaternionWeight),THREE.Quaternion.slerp(k.quaternion,d,k.quaternion,r),m.quaternionWeight+=this.weight)):"scl"===
n&&(c.x=t[0]+(u[0]-t[0])*p,c.y=t[1]+(u[1]-t[1])*p,c.z=t[2]+(u[2]-t[2])*p,r=this.weight/(this.weight+m.scaleWeight),k.scale.lerp(c,r),m.scaleWeight+=this.weight)}}return!0}}}();
...
Math.pow(2,b);Math.pow(4,b);for(var d=f(k.vertices[a.a]),g=f(k.vertices[a.b]),h=f(k.vertices[a.c]),l=[],m=0;m<=c;m++){l[m]=[];for(var n=f(d.clone().lerp(h,m/c)),r=f(g.clone().lerp(h,m/c)),q=c-m,p=0;p<=q;p++)l[m][p]=0==p&&m==c?n:f(n.clone().lerp(r,p/q))}for(m=0;m<c;m++)for(p=0;p<2*(c-m)-1;p++)d=Math.floor(p/2),0==p%2?e(l[m][d+1],l[m+1][d],l[m][d]):e(l[m][d+1],l[m+1][d+1],l[m+1][d])}function h(a,b,c){0>c&&1===a.x&&(a=new THREE.Vector2(a.x-1,a.y));0===b.x&&0===b.z&&(a=new THREE.Vector2(c/2/Math.PI+.5,
