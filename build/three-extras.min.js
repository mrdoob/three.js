...
n+(-3*(b-c)-2*a-d)*e+a*f+b};return function(e){if(!1!==this.isPlaying&&(this.currentTime+=e*this.timeScale,0!==this.weight)){e=this.data.length;if(!0===this.loop&&this.currentTime>e)this.currentTime%=e,this.reset();else if(!1===this.loop&&this.currentTime>e){this.stop();return}e=0;for(var h=this.hierarchy.length;e<h;e++)for(var k=this.hierarchy[e],l=k.animationCache.animations[this.data.name],m=k.animationCache.blending,r=0;3>r;r++){var n=this.keyTypes[r],q=l.prevKey[n],s=l.nextKey[n];if(q&&q[n]&&
s&&s[n]){if(s.time<=this.currentTime){q=this.data.hierarchy[e].keys[0];for(s=this.getNextKeyWith(n,e,1);s.time<this.currentTime&&s.index>q.index;)q=s,s=this.getNextKeyWith(n,e,s.index+1);l.prevKey[n]=q;l.nextKey[n]=s}k.matrixAutoUpdate=!0;k.matrixWorldNeedsUpdate=!0;var p=(this.currentTime-q.time)/(s.time-q.time),w=q[n],y=s[n];0>p&&(p=0);1<p&&(p=1);if("pos"===n)if(this.interpolationType===THREE.AnimationHandler.LINEAR)b.x=w[0]+(y[0]-w[0])*p,b.y=w[1]+(y[1]-w[1])*p,b.z=w[2]+(y[2]-w[2])*p,q=this.weight/
(this.weight+m.positionWeight),k.position.lerp(b,q),m.positionWeight+=this.weight;else{if(this.interpolationType===THREE.AnimationHandler.CATMULLROM||this.interpolationType===THREE.AnimationHandler.CATMULLROM_FORWARD)a[0]=this.getPrevKeyWith("pos",e,q.index-1).pos,a[1]=w,a[2]=y,a[3]=this.getNextKeyWith("pos",e,s.index+1).pos,p=0.33*p+0.33,s=f(a,p),q=this.weight/(this.weight+m.positionWeight),m.positionWeight+=this.weight,n=k.position,n.x+=(s[0]-n.x)*q,n.y+=(s[1]-n.y)*q,n.z+=(s[2]-n.z)*q,this.interpolationType===
THREE.AnimationHandler.CATMULLROM_FORWARD&&(p=f(a,1.01*p),c.set(p[0],p[1],p[2]),c.sub(n),c.y=0,c.normalize(),p=Math.atan2(c.x,c.z),k.rotation.set(0,p,0))}else"rot"===n?(THREE.Quaternion.slerp(w,y,d,p),0===m.quaternionWeight?(k.quaternion.copy(d),m.quaternionWeight=this.weight):(q=this.weight/(this.weight+m.quaternionWeight),THREE.Quaternion.slerp(k.quaternion,d,k.quaternion,q),m.quaternionWeight+=this.weight)):"scl"===n&&(b.x=w[0]+(y[0]-w[0])*p,b.y=w[1]+(y[1]-w[1])*p,b.z=w[2]+(y[2]-w[2])*p,q=this.weight/
(this.weight+m.scaleWeight),k.scale.lerp(b,q),m.scaleWeight+=this.weight)}}return!0}}}();
...
h.faceVertexUvs[0].push([g.clone(),w,H.clone()])}THREE.Geometry.call(this);this.parameters={width:a,height:c,depth:b,widthSegments:d,heightSegments:f,depthSegments:e};this.widthSegments=d||1;this.heightSegments=f||1;this.depthSegments=e||1;var h=this;d=a/2;f=c/2;e=b/2;g("z","y",-1,-1,b,c,d,0);g("z","y",1,-1,b,c,-d,1);g("x","z",1,1,a,b,f,2);g("x","z",1,-1,a,b,-f,3);g("x","y",1,-1,a,c,e,4);g("x","y",-1,-1,a,c,-e,5);this.mergeVertices()};THREE.BoxGeometry.prototype=Object.create(THREE.Geometry.prototype);
...
THREE.CylinderGeometry=function(a,c,b,d,f,e){THREE.Geometry.call(this);this.parameters={radiusTop:a,radiusBottom:c,height:b,radialSegments:d,heightSegments:f,openEnded:e};a=void 0!==a?a:20;c=void 0!==c?c:20;b=void 0!==b?b:100;d=d||8;f=f||1;e=void 0!==e?e:!1;var g=b/2,h,k,l=[],m=[];for(k=0;k<=f;k++){var r=[],n=[],q=k/f,s=q*(c-a)+a;for(h=0;h<=d;h++){var p=h/d,w=new THREE.Vector3;w.x=s*Math.sin(p*Math.PI*2);w.y=-q*b+g;w.z=s*Math.cos(p*Math.PI*2);this.vertices.push(w);r.push(this.vertices.length-1);n.push(new THREE.Vector2(p,
1-q))}l.push(r);m.push(n)}b=(c-a)/b;for(h=0;h<d;h++)for(0!==a?(r=this.vertices[l[0][h]].clone(),n=this.vertices[l[0][h+1]].clone()):(r=this.vertices[l[1][h]].clone(),n=this.vertices[l[1][h+1]].clone()),r.setY(Math.sqrt(r.x*r.x+r.z*r.z)*b).normalize(),n.setY(Math.sqrt(n.x*n.x+n.z*n.z)*b).normalize(),k=0;k<f;k++){var q=l[k][h],s=l[k+1][h],p=l[k+1][h+1],w=l[k][h+1],y=r.clone(),t=r.clone(),H=n.clone(),u=n.clone(),D=m[k][h].clone(),z=m[k+1][h].clone(),x=m[k+1][h+1].clone(),A=m[k][h+1].clone();this.faces.push(new THREE.Face3(q,
s,w,[y,t,u]));this.faceVertexUvs[0].push([D,z,A]);this.faces.push(new THREE.Face3(s,p,w,[t.clone(),H,u.clone()]));this.faceVertexUvs[0].push([z.clone(),x,A.clone()])}if(!1===e&&0<a)for(this.vertices.push(new THREE.Vector3(0,g,0)),h=0;h<d;h++)q=l[0][h],s=l[0][h+1],p=this.vertices.length-1,y=new THREE.Vector3(0,1,0),t=new THREE.Vector3(0,1,0),H=new THREE.Vector3(0,1,0),D=m[0][h].clone(),z=m[0][h+1].clone(),x=new THREE.Vector2(z.x,0),this.faces.push(new THREE.Face3(q,s,p,[y,t,H])),this.faceVertexUvs[0].push([D,
z,x]);if(!1===e&&0<c)for(this.vertices.push(new THREE.Vector3(0,-g,0)),h=0;h<d;h++)q=l[k][h+1],s=l[k][h],p=this.vertices.length-1,y=new THREE.Vector3(0,-1,0),t=new THREE.Vector3(0,-1,0),H=new THREE.Vector3(0,-1,0),D=m[k][h+1].clone(),z=m[k][h].clone(),x=new THREE.Vector2(z.x,1),this.faces.push(new THREE.Face3(q,s,p,[y,t,H])),this.faceVertexUvs[0].push([D,z,x]);this.computeFaceNormals()};THREE.CylinderGeometry.prototype=Object.create(THREE.Geometry.prototype);
...
THREE.LatheGeometry=function(a,c,b,d){THREE.Geometry.call(this);c=c||12;b=b||0;d=d||2*Math.PI;for(var f=1/(a.length-1),e=1/c,g=0,h=c;g<=h;g++)for(var k=b+g*e*d,l=Math.cos(k),m=Math.sin(k),k=0,r=a.length;k<r;k++){var n=a[k],q=new THREE.Vector3;q.x=l*n.x-m*n.y;q.y=m*n.x+l*n.y;q.z=n.z;this.vertices.push(q)}b=a.length;g=0;for(h=c;g<h;g++)for(k=0,r=a.length-1;k<r;k++){c=m=k+b*g;d=m+b;var l=m+1+b,m=m+1,n=g*e,q=k*f,s=n+e,p=q+f;this.faces.push(new THREE.Face3(c,d,m));this.faceVertexUvs[0].push([new THREE.Vector2(n,
q),new THREE.Vector2(s,q),new THREE.Vector2(n,p)]);this.faces.push(new THREE.Face3(d,l,m));this.faceVertexUvs[0].push([new THREE.Vector2(s,q),new THREE.Vector2(s,p),new THREE.Vector2(n,p)])}this.mergeVertices();this.computeFaceNormals();this.computeVertexNormals()};THREE.LatheGeometry.prototype=Object.create(THREE.Geometry.prototype);
THREE.PlaneGeometry=function(a,c,b,d){THREE.Geometry.call(this);this.parameters={width:a,height:c,widthSegments:b,heightSegments:d};var f=a/2,e=c/2;b=b||1;d=d||1;var g=b+1,h=d+1,k=a/b,l=c/d,m=new THREE.Vector3(0,0,1);for(a=0;a<h;a++){var r=a*l-e;for(c=0;c<g;c++)this.vertices.push(new THREE.Vector3(c*k-f,-r,0))}for(a=0;a<d;a++)for(c=0;c<b;c++){var n=c+g*a,f=c+g*(a+1),e=c+1+g*(a+1),h=c+1+g*a,k=new THREE.Vector2(c/b,1-a/d),l=new THREE.Vector2(c/b,1-(a+1)/d),r=new THREE.Vector2((c+1)/b,1-(a+1)/d),q=new THREE.Vector2((c+
1)/b,1-a/d),n=new THREE.Face3(n,f,h);n.normal.copy(m);n.vertexNormals.push(m.clone(),m.clone(),m.clone());this.faces.push(n);this.faceVertexUvs[0].push([k,l,q]);n=new THREE.Face3(f,e,h);n.normal.copy(m);n.vertexNormals.push(m.clone(),m.clone(),m.clone());this.faces.push(n);this.faceVertexUvs[0].push([l.clone(),r,q.clone()])}};THREE.PlaneGeometry.prototype=Object.create(THREE.Geometry.prototype);
THREE.RingGeometry=function(a,c,b,d,f,e){THREE.Geometry.call(this);a=a||0;c=c||50;f=void 0!==f?f:0;e=void 0!==e?e:2*Math.PI;b=void 0!==b?Math.max(3,b):8;d=void 0!==d?Math.max(1,d):8;var g,h=[],k=a,l=(c-a)/d;for(a=0;a<d+1;a++){for(g=0;g<b+1;g++){var m=new THREE.Vector3,r=f+g/b*e;m.x=k*Math.cos(r);m.y=k*Math.sin(r);this.vertices.push(m);h.push(new THREE.Vector2((m.x/c+1)/2,(m.y/c+1)/2))}k+=l}c=new THREE.Vector3(0,0,1);for(a=0;a<d;a++)for(f=a*(b+1),g=0;g<b;g++)e=r=g+f,l=r+b+1,m=r+b+2,this.faces.push(new THREE.Face3(e,
l,m,[c.clone(),c.clone(),c.clone()])),this.faceVertexUvs[0].push([h[e].clone(),h[l].clone(),h[m].clone()]),e=r,l=r+b+2,m=r+1,this.faces.push(new THREE.Face3(e,l,m,[c.clone(),c.clone(),c.clone()])),this.faceVertexUvs[0].push([h[e].clone(),h[l].clone(),h[m].clone()]);this.computeFaceNormals();this.boundingSphere=new THREE.Sphere(new THREE.Vector3,k)};THREE.RingGeometry.prototype=Object.create(THREE.Geometry.prototype);
THREE.SphereGeometry=function(a,c,b,d,f,e,g){THREE.Geometry.call(this);this.parameters={radius:a,widthSegments:c,heightSegments:b,phiStart:d,phiLength:f,thetaStart:e,thetaLength:g};a=a||50;c=Math.max(3,Math.floor(c)||8);b=Math.max(2,Math.floor(b)||6);d=void 0!==d?d:0;f=void 0!==f?f:2*Math.PI;e=void 0!==e?e:0;g=void 0!==g?g:Math.PI;var h,k,l=[],m=[];for(k=0;k<=b;k++){var r=[],n=[];for(h=0;h<=c;h++){var q=h/c,s=k/b,p=new THREE.Vector3;p.x=-a*Math.cos(d+q*f)*Math.sin(e+s*g);p.y=a*Math.cos(e+s*g);p.z=
a*Math.sin(d+q*f)*Math.sin(e+s*g);this.vertices.push(p);r.push(this.vertices.length-1);n.push(new THREE.Vector2(q,1-s))}l.push(r);m.push(n)}for(k=0;k<b;k++)for(h=0;h<c;h++){d=l[k][h+1];f=l[k][h];e=l[k+1][h];g=l[k+1][h+1];var r=this.vertices[d].clone().normalize(),n=this.vertices[f].clone().normalize(),q=this.vertices[e].clone().normalize(),s=this.vertices[g].clone().normalize(),p=m[k][h+1].clone(),w=m[k][h].clone(),y=m[k+1][h].clone(),t=m[k+1][h+1].clone();Math.abs(this.vertices[d].y)===a?(p.x=(p.x+
w.x)/2,this.faces.push(new THREE.Face3(d,e,g,[r,q,s])),this.faceVertexUvs[0].push([p,y,t])):Math.abs(this.vertices[e].y)===a?(y.x=(y.x+t.x)/2,this.faces.push(new THREE.Face3(d,f,e,[r,n,q])),this.faceVertexUvs[0].push([p,w,y])):(this.faces.push(new THREE.Face3(d,f,g,[r,n,s])),this.faceVertexUvs[0].push([p,w,t]),this.faces.push(new THREE.Face3(f,e,g,[n.clone(),q,s.clone()])),this.faceVertexUvs[0].push([w.clone(),y,t.clone()]))}this.computeFaceNormals();this.boundingSphere=new THREE.Sphere(new THREE.Vector3,
...
THREE.TorusKnotGeometry=function(a,c,b,d,f,e,g){function h(a,b,c,d,e){var f=Math.cos(a),g=Math.sin(a);a*=b/c;b=Math.cos(a);f*=d*(2+b)*0.5;g=d*(2+b)*g*0.5;d=e*d*Math.sin(a)*0.5;return new THREE.Vector3(f,g,d)}THREE.Geometry.call(this);this.parameters={radius:a,tube:c,radialSegments:b,tubularSegments:d,p:f,q:e,heightScale:g};a=a||100;c=c||40;b=b||64;d=d||8;f=f||2;e=e||3;g=g||1;for(var k=Array(b),l=new THREE.Vector3,m=new THREE.Vector3,r=new THREE.Vector3,n=0;n<b;++n){k[n]=Array(d);var q=n/b*2*f*Math.PI,
s=h(q,e,f,a,g),q=h(q+0.01,e,f,a,g);l.subVectors(q,s);m.addVectors(q,s);r.crossVectors(l,m);m.crossVectors(r,l);r.normalize();m.normalize();for(q=0;q<d;++q){var p=q/d*2*Math.PI,w=-c*Math.cos(p),p=c*Math.sin(p),y=new THREE.Vector3;y.x=s.x+w*m.x+p*r.x;y.y=s.y+w*m.y+p*r.y;y.z=s.z+w*m.z+p*r.z;k[n][q]=this.vertices.push(y)-1}}for(n=0;n<b;++n)for(q=0;q<d;++q)f=(n+1)%b,e=(q+1)%d,a=k[n][q],c=k[f][q],f=k[f][e],e=k[n][e],g=new THREE.Vector2(n/b,q/d),l=new THREE.Vector2((n+1)/b,q/d),m=new THREE.Vector2((n+1)/
b,(q+1)/d),r=new THREE.Vector2(n/b,(q+1)/d),this.faces.push(new THREE.Face3(a,c,e)),this.faceVertexUvs[0].push([g,l,r]),this.faces.push(new THREE.Face3(c,f,e)),this.faceVertexUvs[0].push([l.clone(),m,r.clone()]);this.computeFaceNormals();this.computeVertexNormals()};THREE.TorusKnotGeometry.prototype=Object.create(THREE.Geometry.prototype);
THREE.TubeGeometry=function(a,c,b,d,f){THREE.Geometry.call(this);this.parameters={path:a,segments:c,radius:b,radialSegments:d,closed:f};c=c||64;b=b||1;d=d||8;f=f||!1;var e=[],g,h,k=c+1,l,m,r,n,q=new THREE.Vector3,s,p,w;s=new THREE.TubeGeometry.FrenetFrames(a,c,f);p=s.normals;w=s.binormals;this.tangents=s.tangents;this.normals=p;this.binormals=w;for(s=0;s<k;s++)for(e[s]=[],l=s/(k-1),n=a.getPointAt(l),g=p[s],h=w[s],l=0;l<d;l++)m=l/d*2*Math.PI,r=-b*Math.cos(m),m=b*Math.sin(m),q.copy(n),q.x+=r*g.x+m*
h.x,q.y+=r*g.y+m*h.y,q.z+=r*g.z+m*h.z,e[s][l]=this.vertices.push(new THREE.Vector3(q.x,q.y,q.z))-1;for(s=0;s<c;s++)for(l=0;l<d;l++)k=f?(s+1)%c:s+1,q=(l+1)%d,a=e[s][l],b=e[k][l],k=e[k][q],q=e[s][q],p=new THREE.Vector2(s/c,l/d),w=new THREE.Vector2((s+1)/c,l/d),g=new THREE.Vector2((s+1)/c,(l+1)/d),h=new THREE.Vector2(s/c,(l+1)/d),this.faces.push(new THREE.Face3(a,b,q)),this.faceVertexUvs[0].push([p,w,h]),this.faces.push(new THREE.Face3(b,k,q)),this.faceVertexUvs[0].push([w.clone(),g,h.clone()]);this.computeFaceNormals();
...
a.y));return a.clone()}THREE.Geometry.call(this);b=b||1;d=d||0;for(var k=this,l=0,m=a.length;l<m;l+=3)f(new THREE.Vector3(a[l],a[l+1],a[l+2]));a=this.vertices;for(var r=[],n=l=0,m=c.length;l<m;l+=3,n++){var q=a[c[l]],s=a[c[l+1]],p=a[c[l+2]];r[n]=new THREE.Face3(q.index,s.index,p.index,[q.clone(),s.clone(),p.clone()])}for(var w=new THREE.Vector3,l=0,m=r.length;l<m;l++)g(r[l],d);l=0;for(m=this.faceVertexUvs[0].length;l<m;l++)c=this.faceVertexUvs[0][l],d=c[0].x,a=c[1].x,r=c[2].x,n=Math.max(d,Math.max(a,
