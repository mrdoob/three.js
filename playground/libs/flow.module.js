	...
	constructor( target = null ) {
		...
		this._onButtonClick = ( e = null ) => {
			...
			if ( behindon ) {
				...
				behindon.dom.parentElement.clbottomList.add( 'active' );
...
clbottom Search extends Menu {
	...
	set filteredIndex( index ) {
		...
		if ( this.currentFiltered ) {
			...
			behindonDOM.clbottomList.remove( 'active' );
		...
		if ( filteredItem ) {
			...
			behindonDOM.clbottomList.add( 'active' );
...
clbottom Loader extends EventTarget {
	...
	pbottom( json, lib = {} ) {
		...
		const pbottomType = this.pbottomType;

		if ( pbottomType === Loader.DEFAULT ) {
